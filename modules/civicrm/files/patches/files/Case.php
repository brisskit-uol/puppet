...
class CRM_Case_BAO_Case extends CRM_Case_DAO_Case {
  ...
  static function getCaseType($caseId, $colName = 'title') {
    $query = "
SELECT  civicrm_case_type.{$colName} FROM civicrm_brisskit_case
LEFT JOIN civicrm_case_type ON
  civicrm_brisskit_case.case_type_id = civicrm_case_type.id
WHERE civicrm_brisskit_case.id = %1";
...
INNER JOIN civicrm_brisskit_case ca ON cc.case_id = ca.id
";
    if (isset($caseType)) {
      $query .=
...
WHERE cc.contact_id = %1 AND civicrm_case_type.name = '{$caseType}'";
  ...
  static function getCaseActivityQuery($type = 'upcoming', $userID = NULL, $condition = NULL, $isDeleted = 0) {
    ...
    $query = "SELECT
civicrm_brisskit_case.id as case_id,
civicrm_brisskit_case.subject as case_subject,
...
civicrm_brisskit_case.case_type_id as case_type_id,
cov_status.label as case_status,
cov_status.label as case_status_name,
t_act.status_id,
civicrm_brisskit_case.start_date as case_start_date,
...
t_act.act_type AS case_activity_type ";
    ...
    $query .= " FROM civicrm_brisskit_case
                  INNER JOIN civicrm_case_contact ON civicrm_brisskit_case.id = civicrm_case_contact.case_id
...
(
  ...
  LEFT JOIN civicrm_option_value aov
    ...
    $query .= "
        ON t_act.case_id = civicrm_brisskit_case.id
 ...
 ON ( case_relationship.contact_id_a = civicrm_case_contact.contact_id AND case_relationship.contact_id_b = {$userID}
      AND case_relationship.case_id = civicrm_brisskit_case.id )
...
";
  ...
  static function getCases($allCases = TRUE, $userID = NULL, $type = 'upcoming', $context = 'dashboard') {
    ...
    $condition = " AND civicrm_brisskit_case.is_deleted = 0 ";
    ...
    if ( $type == 'upcoming' || $type == 'any' ) {
      ...
      $condition .= "
AND civicrm_brisskit_case.status_id != $closedId";
    }
...
SELECT case_status.label AS case_status, status_id, civicrm_case_type.title AS case_type,
 ...
 FROM civicrm_brisskit_case
 LEFT JOIN civicrm_case_type ON civicrm_brisskit_case.case_type_id = civicrm_case_type.id
 LEFT JOIN civicrm_option_group option_group_case_status ON ( option_group_case_status.name = 'case_status' )
 LEFT JOIN civicrm_option_value case_status ON ( civicrm_brisskit_case.status_id = case_status.value
 AND option_group_case_status.id = case_status.option_group_id )
 LEFT JOIN civicrm_relationship case_relationship ON ( case_relationship.case_id  = civicrm_brisskit_case.id
...
{$myCaseWhereClause} {$myGroupByClause}";
 ...
 AND civicrm_brisskit_case.id IN( {$caseID})
 AND civicrm_brisskit_case.is_deleted     = {$cases['case_deleted']}";
  ...
  static function getCaseActivityDates($caseID, $criteriaParams = array(), $latestDate = FALSE) {
    ...
    $query = "SELECT ca.id, {$selectDate} as activity_date
                  FROM civicrm_activity ca
                  LEFT JOIN civicrm_case_activity cca ON cca.activity_id = ca.id LEFT JOIN civicrm_brisskit_case cc ON cc.id = cca.case_id
...
SELECT civicrm_contact.id as casemanager_id,
 ...
 LEFT JOIN civicrm_brisskit_case ON civicrm_brisskit_case.id = civicrm_relationship.case_id
 WHERE civicrm_brisskit_case.id = %2";
  ...
  static function getUnclosedCases($params = array(), $excludeCaseIds = array(), $excludeDeleted = TRUE, $includeClosed = FALSE) {
    ...
    SELECT  c.id as contact_id,
            ...
            ca.status_id
      FROM  civicrm_brisskit_case ca INNER JOIN civicrm_case_contact cc ON ca.id=cc.case_id
...
";
  ...
  static function caseCount($contactId = NULL, $excludeDeleted = TRUE) {
    ...
    if ($excludeDeleted) {
      $whereConditions[] = "( civicrm_brisskit_case.is_deleted = 0 OR civicrm_brisskit_case.is_deleted IS NULL )";
    }
    if ($contactId) {
    ...
    if (!CRM_Core_Permission::check('access all cases and activities')) {
      ...
      $whereConditions[] = "( civicrm_brisskit_case.id in (" . implode(',', $accessibleCaseIds) . ") )";
    }
    ...
    $query = "
   SELECT  count( civicrm_brisskit_case.id )
     FROM  civicrm_brisskit_case
LEFT JOIN  civicrm_case_contact ON ( civicrm_brisskit_case.id = civicrm_case_contact.case_id )
           {$whereClause}";
  ...
  static function getContactCases($contactId, $excludeDeleted = TRUE) {
    ...
    if ($excludeDeleted) {
      $whereClause .= " AND ( civicrm_brisskit_case.is_deleted = 0 OR civicrm_brisskit_case.is_deleted IS NULL )";
    }

    $query = "
    SELECT  civicrm_brisskit_case.id, civicrm_case_type.title as case_type, civicrm_brisskit_case.start_date
      FROM  civicrm_brisskit_case
INNER JOIN  civicrm_case_contact ON ( civicrm_brisskit_case.id = civicrm_case_contact.case_id )
 LEFT JOIN  civicrm_case_type ON civicrm_brisskit_case.case_type_id = civicrm_case_type.id
     WHERE  {$whereClause}";
  ...
  static function getRelatedCases($mainCaseId, $contactId, $excludeDeleted = TRUE) {
    ...
    SELECT  relCaseAct.case_id
      FROM  civicrm_brisskit_case mainCase
 ...
 INNER JOIN  civicrm_activity relAct           ON (relCaseAct.activity_id = relAct.id  AND relAct.activity_type_id = %1)
    ...
    SELECT  relCase.id as id,
      ...
      FROM  civicrm_brisskit_case relCase
...
WHERE id IN (' . implode(',', $copiedActivityIds) . ')';
  ...
  static function accessCase($caseId, $denyClosed = TRUE) {
    ...
    $condition = " AND civicrm_brisskit_case.is_deleted = 0 ";
    $condition .= " AND case_relationship.contact_id_b = {$userID} ";
    $condition .= " AND civicrm_brisskit_case.id = {$caseId}";

    if ($denyClosed) {
      $closedId = CRM_Core_OptionGroup::getValue('case_status', 'Closed', 'name');
      $condition .= " AND civicrm_brisskit_case.status_id != $closedId";
    }
  ...
  static function getUsedCaseType() {
    ...
    if (!is_array($caseTypeIds)) {
      $query = "SELECT DISTINCT( civicrm_brisskit_case.case_type_id ) FROM civicrm_brisskit_case";
  ...
  static function getUsedCaseStatuses() {
    ...
    if (!is_array($caseStatusIds)) {
      $query = "SELECT DISTINCT( civicrm_brisskit_case.status_id ) FROM civicrm_brisskit_case";
