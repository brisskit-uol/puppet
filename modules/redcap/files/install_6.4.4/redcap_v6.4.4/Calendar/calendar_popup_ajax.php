...
if ($isAjax) $event_date = DateTimeRC::format_ts_to_ymd($event_date);
...
if (isset($_REQUEST['action'])) {
	switch ($_REQUEST['action']) {	
		//Edit the visit date
		case "edit_date":
			$sql = "update redcap_events_calendar set event_date = '".prep($event_date)."' where cal_id = '".prep($_GET['cal_id'])."'";
			$q = db_query($sql);
			if (!$q) {
