	...
	{	
		...
		case "adddates":
			...
			if (!isset($_GET['display_only'])) {
				...
				if ($already_sched) {
					exit("<br><div><span class='red'><b>{$lang['global_01']}{$lang['colon']}</b> $table_pk_label \"<b>".removeDDEending($_GET['idnumber'])."</b>\" ".($Proj->numArms > 1 ? $lang['scheduling_84'] : $lang['scheduling_28'])."</span></div>");
				}			
				
				//Display confirmation text
				print  "<div class='darkgreen' style='font-family:arial;padding:15px 15px;'>";
				print  "<h3 style='margin:0 0 15px 0;color:green;font-family:verdana;'><img src='".APP_PATH_IMAGES."tick.png'> {$lang['scheduling_29']} \"".removeDDEending($_GET['idnumber'])."\"</h3>";
				print  "<p>
							$table_pk_label \"<b>".removeDDEending($_GET['idnumber'])."</b>\" {$lang['scheduling_30']} 
				...
				for ($i = 0; $i < count($dates); $i++) {
					...
					if ($dates[$i] != "" && checkdate($start_month,$start_day,$start_year)) {
						...
						$sql = "insert into redcap_events_calendar (record, project_id, group_id, event_id, event_date, event_time, 
								event_status, baseline_date) values ('".prep($_GET['idnumber'])."', $project_id, " . checkNull($group_id) . ", '".prep($event_ids[$i])."', 
								'".prep($dates[$i])."', '".prep($times[$i])."', 0, '".prep(DateTimeRC::format_ts_to_ymd(trim($_GET['baseline_date'])))."')";
						if (db_query($sql)) {
							$sql_all[] = $sql;
						} else {
							$sql_errors[] = $sql;
			...
			print  "<br><table class='form_border' style='color:#000;'>
					<tr>
						<td class='label_header' colspan='3' style='padding:8px 5px 3px 8px;background-color:#d5d5d5;font-size:14px;border:1px solid #bbb;'>
							<div>
								{$lang['scheduling_33']} \"".removeDDEending($_GET['idnumber'])."\" $extra_record_label
							</div>";
			if (PAGE != "ProjectGeneral/print_page.php") {
				print  "	<div style='text-align:right;font-weight:normal;'>
								<img src='".APP_PATH_IMAGES."pencil.png' class='imgfix'> <a href='".APP_PATH_WEBROOT."Calendar/scheduling.php?pid=$project_id&record=".removeDDEending($_GET['idnumber'])."&arm=".getArm()."' style='text-decoration:underline;'>{$lang['global_27']}</a> 
								&nbsp;
								<img src='".APP_PATH_IMAGES."printer.png' class='imgfix'> <a href='javascript:;' onclick=\"window.open('".APP_PATH_WEBROOT."ProjectGeneral/print_page.php?pid=$project_id&action=adddates&schedule&display_only&idnumber=".removeDDEending($_GET['idnumber'])."&arm=".getArm()."','myWin','width=850, height=800, toolbar=0, menubar=1, location=0, status=0, scrollbars=1, resizable=1');\" style='text-decoration:underline;'>{$lang['scheduling_35']}</a>
							</div>";
			}
			print  "		</td>
					<tr>
		...
		case 'generate_sched':
				...
				if (db_num_rows($q) > 0) {
					...
					if (db_num_rows($q) < 1) {
						...
						print  "<div class='red'>
									...
									<b>$table_pk_label ".removeDDEending($_GET['idnumber'])." {$lang['scheduling_39']}</b><br><br>
			...
			print  "<div style='font-size:16px;font-weight:bold;margin:0 0 15px 0;font-family:verdana;'>
						{$lang['scheduling_42']} \"".removeDDEending($_GET['idnumber'])."\"&nbsp;&nbsp;
						<span style='color:#800000;font-size:11px;font-family:arial;'>({$lang['global_02']}: {$lang['scheduling_43']})</span>
					</div>";

			print  "<p>{$lang['scheduling_44']} <b>$table_pk_label</b> \"<b>".removeDDEending($_GET['idnumber'])."</b>\" 
					{$lang['scheduling_45']}";
			...
			while ($row = db_fetch_assoc($q)) {
				...
				$this_event_date = date(DateTimeRC::get_user_format_php(), mktime(0, 0, 0, $start_month, $start_day + $row['day_offset'], $start_year));
				$this_visit_day  = date("l", 	 mktime(0, 0, 0, $start_month, $start_day + $row['day_offset'], $start_year));
				$this_offset_min = date(DateTimeRC::get_user_format_php(), mktime(0, 0, 0, $start_month, $start_day + $row['day_offset'] - $row['offset_min'], $start_year));
				$this_offset_max = date(DateTimeRC::get_user_format_php(), mktime(0, 0, 0, $start_month, $start_day + $row['day_offset'] + $row['offset_max'], $start_year));
			...
			print  "<p>";
			print  "<input type='button' value='Create Schedule' id='createbtn' onclick=\"createSched('".cleanHtml(removeDDEending($_GET['idnumber']))."',{$_GET['newid']});\">&nbsp;
			...
			if ($_GET['newid']) {
				print  "<div style='font-size:11px;padding-top:4px;color:#555;'>
						{$lang['global_02']}: {$lang['scheduling_59']} <i style='color:#800000;'>{$lang['scheduling_03']}</i> {$lang['scheduling_60']} \"<b>".removeDDEending($_GET['idnumber'])."</b>\" 
		...
		case 'edit_single':
			...
			$sql1 = "update redcap_events_calendar set event_date = '".DateTimeRC::format_ts_to_ymd(trim($_GET['event_date']))."', 
					 event_time = '".prep($_GET['event_time'])."', event_status = '".prep($_GET['event_status'])."', 
					 notes = '".prep($_GET['notes'])."' where cal_id = '".prep($_GET['cal_id'])."'";
			db_query($sql1);
			//LOGGING for single
			...
			if ($_GET['other_rows'] != "") {
				...
				$sql2 = "update redcap_events_calendar set event_date = date_add(event_date, INTERVAL ".prep($_GET['daydiff'])." DAY) 
...
print  "<h3 id='view_edit_title' style='margin:0 0 15px 0;font-family:verdana;'>
			{$lang['scheduling_62']} \"".RCView::escape(removeDDEending($_GET['record']))."\" $extra_record_label
		</h3>";
...
print  "<div id='new_ad_hoc'>
			<div style='color:green;font-size:12px;padding:15px 0 0;'>
				...
				{$lang['scheduling_81']}
				&nbsp;<input type='text' id='newCalEv' class='x-form-text x-form-field cal2' style='width:70px;' maxlength='10' value='" . DateTimeRC::format_ts_from_ymd(TODAY) . "' onblur=\"redcap_validate(this,'','','hard','date_'+user_date_format_validation,1,1,user_date_format_delimiter);\">
				<input type='button' id='btn_newCalEv' value=' ".cleanHtml($lang['design_171'])." ' class='imgfix' onclick=\"
					...
					popupCalNew(date_arr[2],(date_arr[1]*1+1),date_arr[0],'".cleanHtml(str_replace(array("'","\""),array("","&quot;"),$_GET['record']))."');			
				\">
			</div>
