...
if (!isset($_POST['action']))
{
	...
	{		
		...
		foreach ($endSurveyNotify as $this_user=>$attr) 
		{
			...
			if (empty($user_emails)) {
				...
				$user_email_dropdown = RCView::select(array('class'=>'x-form-text x-form-field', 'style'=>'max-width:100%;width:100%;padding-right:0;height:22px;', 
										'onchange'=>"endSurvTrigSave('".cleanHtml($this_user)."',this.value,$this_survey_id);"), $user_emails, $attr['surveys'][$this_survey_id]);
			}
			// Build row for user
			$r .=	RCView::tr('',					
						...
						RCView::td(array('class'=>'data','style'=>'padding:4px 2px;text-align:center;color:red;font-size:11px;font-family:arial;font-weight:bold;'),
							// Save status span
		...
		$h .= 	RCView::table(array('cellspacing'=>'0','class'=>'form_border','style'=>'table-layout:fixed;margin:20px 0;width:100%;'),
					...
					RCView::tr('',					
						...
						RCView::td(array('class'=>'header','style'=>'font-size:11px;text-align:center;font-weight:normal;width:60px;'),
							$lang['setup_126']
						)
					) .
					// All rows of users
	...
	exit(json_encode(array('title'=>$t, 'content'=>$h)));
}
...
if ($_POST['action'] == "endsurvey_email" && is_numeric($_POST['value']))
{
	...
	// Send back JSON with info
	exit(json_encode(array('survey_notifications_enabled'=>(Survey::surveyNotificationsEnabled() ? '1' : '0'))));
}
