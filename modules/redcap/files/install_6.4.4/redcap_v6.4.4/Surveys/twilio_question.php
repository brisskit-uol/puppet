...
$isPromisInstrument = PROMIS::isPromisInstrument($form_name);


// Settings for SMS and Voice
if (SMS) {
	// Set the number to send SMS to
	$number_to_sms = $_POST['From'];
} else {
...
if (isset($_SESSION['response_id']) && is_numeric($_SESSION['response_id'])) {
	// Record exists: If response_id is in URL, then use it to obtain record name
	$sql = "select record from redcap_surveys_response where response_id = {$_SESSION['response_id']} 
	...
	if ($public_survey) {
		...
		$participant_ids = getRecordFromPartId(array($participant_id));
		if ($participant_ids[$participant_id] == '') {
			...
			if (db_num_rows($q)) {
				...
				if ($row['completion_time'] != '') exitSurvey($lang['survey_111']);
...
## GET DATA IF AN EXISTING RECORD AND IF SURVEY HAS BRANCHING LOGIC
// Put record data in array (for branching logic uses only)
$record_data = array();
if ($record_exists) {
	// Record exists
	if (isset($Proj->forms[$form_name]) && $Proj->forms[$form_name]['has_branching']) {
		// Get record data for whole record (because we might need it for multiple fields if we loop through much branching logic in this same response)
...
if ($answer != null && isset($_SESSION['field']) && $_SESSION['field'] != '' && isset($Proj->metadata[$_SESSION['field']]))
{
	...
	if ($Proj->isMultipleChoice($_SESSION['field'])) 
	{
		...
		if (!isset($fieldChoices[$answer])) {
			...
			if (SMS) {
				TwilioRC::sendSMS($lang['survey_681'], $number_to_sms, $twilioClient);
			} else {
			...
			&& $Proj->metadata[$_SESSION['field']]['element_validation_type'] != '')
	{
		...
		elseif ($Proj->metadata[$_SESSION['field']]['element_validation_min'] != '' || $Proj->metadata[$_SESSION['field']]['element_validation_max'] != '') {
			...
			if (!$saveData) {
				// An invalid choice was entered
				if (SMS) {
					TwilioRC::sendSMS($lang['config_functions_57'] . " $out_of_range_lang", $number_to_sms, $twilioClient);
				} else {
	...
	if ($saveData)
	{
		...
		if (isset($_SESSION['response_id']) && is_numeric($_SESSION['response_id'])) {
			...
			$sql .= "record = '" . prep($fetched) . "' limit 1";
		}
		$q = db_query($sql);
		if ($q && db_num_rows($q) > 0) {
			...
			$sql = "insert into redcap_surveys_response (participant_id, record, first_submit_time) 
...
if ($field == null) {
	...
	$instructions = trim(strip_tags(label_decode($Proj->surveys[$survey_id]['instructions'])));
	if (!isset($_SESSION['field']) && $instructions != '') {
		if ($record_exists) {
			$instructions = Piping::replaceVariablesInLabel($instructions, $fetched, $event_id, $record_data);
		}
		// Pause initially to give participant time to pick up and begin hearing
		if (SMS) {
			TwilioRC::sendSMS($instructions, $number_to_sms, $twilioClient);
		} else {
			$twiml->pause(""); $twiml->pause("");
	...
	if ($nextField !== false) 
	{
		...
		if (!$isPromisInstrument) 
		{
			do {			
				...
				if (!$skipField && $record_exists && $branching_logic != '') {
					if ($longitudinal) {
						$branching_logic = LogicTester::logicPrependEventName($branching_logic, $Proj->getUniqueEventNames($event_id));
					}
				...
				elseif ($isDescriptiveField) {			
					...
					if ($Proj->metadata[$field]['element_preceding_header'] != '') {
						if (SMS) {
							TwilioRC::sendSMS($Proj->metadata[$field]['element_preceding_header'], $number_to_sms, $twilioClient);
						} else {
							$twiml->say(strip_tags($Proj->metadata[$field]['element_preceding_header']), $say_array);
						}
					}
					// Say descriptive label
					if (SMS) {
						TwilioRC::sendSMS($Proj->metadata[$field]['element_label'], $number_to_sms, $twilioClient);
					} else {
						$twiml->say(strip_tags($Proj->metadata[$field]['element_label']), $say_array);
					}
					// Get next field for next loop (if not at end of survey)
					$field = $Proj->getNextField($field);
				}
		...
		if ($field == $form_name."_complete") 
		{
			// Send SMS text thanking them right before hang-un
			// $twiml->sms("Thanks for taking our survey! To take the survey again, call ".($public_survey ? $_GET['ToFormatted'] : $_GET['FromFormatted']));
			...
			$acknowledgement = trim(strip_tags(Piping::replaceVariablesInLabel(label_decode($Proj->surveys[$survey_id]['acknowledgement']), $fetched, $event_id)));
			if ($acknowledgement != '') {
				if (SMS) {
					TwilioRC::sendSMS($acknowledgement, $number_to_sms, $twilioClient);
				} else {
			...
			if ($surveyQueueEnabled && !empty($survey_queue_items)) {
				...
				foreach ($survey_queue_items as $queueAttr) {
					...
					if (SMS) {
						...
						// Redirect
						TwilioRC::addSmsAccessCodeForPhoneNumber($number_to_sms, $twilio_from_number, $access_code);
						// Redirect to next survey if reply with any text
						sleep(2);
						TwilioRC::sendSMS($lang['survey_900'], $number_to_sms, $twilioClient);
					} else {
						// Redirect to next survey if press any key
						$twiml->pause("");
						$gather = $twiml->gather(array('method'=>'POST', 'action'=>APP_PATH_SURVEY_FULL . "?s={$queueAttr['hash']}", 'timeout'=>5, 'finishOnKey'=>'#'));
						// Say the label(s)
...
if ($field !== false) 
{
	...
	// Build the label(s)
	$label = strip_tags($Proj->metadata[$field]['element_label']) . " ";
	...
	if (SMS && in_array($data_type, array('date', 'datetime', 'datetime_seconds'))) {
		// Date/time fields (for SMS only)
		$label .= "(" . MetaData::getDateFormatDisplay($val_type) . ")";
	} elseif ($Proj->metadata[$field]['element_type'] == 'slider') {
		// Slider
		$label .= $lang['survey_898'];
		// Set max digits as 3 (because max value is 100)
		$choices_max_digits = 3;
	} elseif ($Proj->metadata[$field]['element_type'] == 'truefalse') {
		// TF
		$label .= $lang['survey_896'];
	} elseif ($Proj->metadata[$field]['element_type'] == 'yesno') {
		// YN
		$label .= $lang['survey_897'];
		...
		foreach ($this_enum_array as $code=>$this_label) {
			...
			$this_label = strip_tags($this_label);
			// Special formatting for CATs
			if ($isPromisInstrument) {
				if (SMS) {
	...
	if ($record_exists) {
		$label = Piping::replaceVariablesInLabel($label, $fetched, $event_id, $record_data);
	}
	...
	if (SMS) 
	{
		...
		TwilioRC::sendSMS($label, $number_to_sms, $twilioClient);
	} 
	// VOICE question
	elseif (VOICE) 
	{
		...
		$gather_params = array('method'=>'POST', 'action'=>$question_url, 'timeout'=>3, 'finishOnKey'=>'#', 'numDigits'=>$choices_max_digits);
