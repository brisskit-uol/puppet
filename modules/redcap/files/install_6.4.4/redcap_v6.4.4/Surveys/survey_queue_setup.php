...
elseif (isset($_POST['action']) && $_POST['action'] == "save")
{
	...
	foreach ($_POST as $key=>$val) {
		...
		if ($conditionLogic == '' && ($surveyCompSurveyId == '' || $surveyCompEventId == '')) {
			...
			$sql_all[] = $sql = "delete from redcap_surveys_queue where survey_id = '".prep($survey_id)."' and event_id = '".prep($event_id)."'";
		} else {
			// Insert/update
			$sql_all[] = $sql = "insert into redcap_surveys_queue (survey_id, event_id, active, condition_surveycomplete_survey_id, condition_surveycomplete_event_id,
					...
					('".prep($survey_id)."', '".prep($event_id)."', $active, ".checkNull($surveyCompSurveyId).", ".checkNull($surveyCompEventId).", '$andOr', 
	...
	$popupContent = RCView::img(array('src'=>'tick.png', 'class'=>'imgfix')) . 
...
print json_encode(array('content'=>$popupContent, 'title'=>$popupTitle, 'survey_queue_enabled'=>$survey_queue_enabled));