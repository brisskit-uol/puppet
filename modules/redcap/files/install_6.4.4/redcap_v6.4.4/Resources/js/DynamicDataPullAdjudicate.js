...
function showHiddenAdjudRows(hidethem) {
	...
	if (!hidethem) {	
		...
		$('#rtws_adjud_form tr.hidden').each(function(){
...
function triggerRTWSmappedField(record_name, autoOpenDialog, outputHtml, showExclusions, forceDataFetch) {
	...
	adjud_dialog = $('#rtws_adjud_popup').dialog({ bgiframe: true, modal: true, autoOpen: autoOpenDialog, width: 1000, close: function(){ $(this).dialog('destroy'); } });
	...
	// Call web service via ajax
	$.post(app_path_webroot+'DynamicDataPull/fetch.php?pid='+pid+'&event_id='+event_id+'&day_offset='+day_offset
		   +'&day_offset_plusminus='+encodeURIComponent(day_offset_plusminus)+'&output_html='+outputHtml+'&record_exists='+record_exists2
		   +'&show_excluded='+(showExclusions?'1':'0')+'&forceDataFetch='+(forceDataFetch?'1':'0'), params, function(data){
		// Parse the JSON
		...
		if (itemsToAdjudicate == -2) {
			simpleDialog(json_data.html,'ERROR');
			return;
		}
		...
		if (outputHtml) {
			...
			if ($('#rtws_adjud_popup_content #adjud_hide_buttons').length && $('#rtws_adjud_popup_content #adjud_hide_buttons').text() == '1') {
				...
				$('#rtws_adjud_popup').dialog('option', 'buttons', [
					...
					{ text: 'Save', click: function() { 
						var rtws_adjud_form_vars = $('form#rtws_adjud_form').serializeObject();
						if ($('form#rtws_adjud_form input[type="radio"]').serialize().length == 0) {
							simpleDialog("No fields were selected! You must first select one or more values from the source system before they can be imported into REDCap.");
							return;
						}
						// Close dialog
						$(this).dialog('close');
						// Save data via ajax
						$.post(app_path_webroot+'DynamicDataPull/save.php?pid='+pid+'&record='+record_name+'&event_id='+event_id, rtws_adjud_form_vars, function(data){
			...
			$('#rtws_adjud_popup .rtws_adjud_radio').click(function(){
				...
				if (this_row_class != null) {
					$('#adjud_table tr.'+this_row_class).find('.rtws_adjud_radio').attr('checked', false);
		...
		if (!autoOpenDialog && (page == 'DataEntry/index.php' || page == 'Mobile/data_entry.php')) {
			...
			if (itemsToAdjudicate < 0) {
				...
				// No new items
				var newItemsText = '<img src="'+app_path_images+'information_frame.png" class="imgfix"> <span style="color:#000066;">No new items from source system</span>';
			} else {
				...
				var newItemsText = '<div id="RTWS_sourceDataCheck_msgBox" class="red" style="display:none;color:#C00000;text-align:center;font-weight:bold;">'
								 + '<span class="badge" style="font-size:12px;">'+itemsToAdjudicate+'</span><span style="" class="imgfix1">new items from source system</span>'
			...
			if ($('#RTWS_sourceDataCheck #RTWS_sourceDataCheck_msgBox').length) {
				...
				if (itemsToAdjudicate != $('#RTWS_sourceDataCheck #RTWS_sourceDataCheck_msgBox .badge').text()*1) {
					$('#RTWS_sourceDataCheck #RTWS_sourceDataCheck_msgBox .badge').html(itemsToAdjudicate);
					setTimeout(function(){
...
function openAdjudicationDialog(record_name) {
	if ($('#rtws_adjud_popup').dialog('isOpen') == false) {
...
function radioResetValAdjud(field,form) {
	uncheckRadioGroup(document.forms[form].elements[field]);
	resetAdjudRadioBgLink(field);
}
// Reset bgcolor for all radios in this group and hide "reset" link
function resetAdjudRadioBgLink(field) {
	$('#rtws_adjud_popup input[name="'+field+'"]').each(function(){
...
function autoCheckNewItemsFromSource(record, forceDataFetch) {
	...
	var fetchingDataText = '<img src="'+app_path_images+'progress_circle.gif" class="imgfix"> Checking data in source system...';
...
function setRTWSContextMsgPlaceholder(text) {
	...
	if (!$('div#contextMsg #RTWS_sourceDataCheck').length) {
		...
		contextMsgDiv.html('<table cellspacing="0" style="width:100%;table-layout:fixed;"><tr><td id="RTWS_contextMsg_original">'+contextMsgContents+'</td>'
						 + '<td id="RTWS_sourceDataCheck" style="color:#666;width:300px;text-align:right;">'+text+'</td></tr></table>');
...
function addDDPicon(field) {
	$('table#form_table #'+field+'-tr').children('td:last').css('background','#F3F3F3 url("'+app_path_images+'databases_arrow.png") no-repeat 97% 8px');
}
