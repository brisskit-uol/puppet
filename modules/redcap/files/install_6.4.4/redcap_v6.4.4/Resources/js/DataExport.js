...
function fetchReportAjax(report_id,pagenum) {
	...
	// Ajax call
	exportajax = $.post(app_path_webroot+'DataExport/report_ajax.php?pid='+pid+getInstrumentsListFromURL()+'&pagenum='+pagenum, { report_id: report_id }, function(data) {
		if (data == '0' || data == '') {
...
function showExportFormatDialog(report_id) {
	...
	$('#exportFormatDialog').dialog({ title: title, bgiframe: true, modal: true, width: 870, open: function(){ fitDialog(this) }, buttons: 
		...
		{text: exportBtnTxt, click: function() {
			...
			exportajax = $.post(app_path_webroot+'DataExport/data_export_ajax.php?pid='+pid+getSelectedInstrumentList()+getInstrumentsListFromURL(), params, function(data) {
				if (data == '0' || data == '') {
					showProgressExport(0,0);
...
function enableAutoSuggestFields() {
	$('table#create_report_table .field-auto-suggest').each(function(){
		...
		if (obId == null) {
			...
			ob.attr('id', obId);
			// Enable auto suggest
			$('#'+obId).ui_autocomplete({ delay: 0, source: autoSuggestFieldList, minLength: 1,
...
function getUserAccessList() {
	...
	$.post(app_path_webroot+'DataExport/report_user_access_list.php?pid='+pid, $('form#create_report_form').serializeObject(), function(data) {
		if (data == '0') {
			alert(woops);
			return;
		}
...
function updateReportPanel() {
	$.post(app_path_webroot+'DataExport/render_report_panel_ajax.php?pid='+pid, { }, function(data){
		$('#report_panel').remove();
		if (data != '') {
...
function loadReportNewPage(pagenum) {
	...
	modifyURL(app_path_webroot+'DataExport/index.php?pid='+pid+'&report_id='+report_id+(report_id=='SELECTED' ? '&instruments='+getParameterByName('instruments') : '')+'&pagenum='+pagenum);
