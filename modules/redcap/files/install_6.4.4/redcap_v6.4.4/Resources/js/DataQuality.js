...
function enableRuleTableEdit() {
	...
	$('#table-rules .editrte').hover(function(){
...
function executeRulesAjax(rule_ids,show_exclusions,replaceRuleTable,action) {
	...
	// Ajax request
	$.post(app_path_webroot+'DataQuality/execute_ajax.php?pid='+pid, { record: $('#dqRuleRecord').val(), rule_ids: rule_ids, show_exclusions: show_exclusions, action: action }, function(data){
		// Get data returned
		var json_data = jQuery.parseJSON(data);
		if (json_data.length < 1) {
			alert(woops);
		...
		if (replaceRuleTable == 0) {
			...
			var discrep_text = "<div style='float:left;font-size:15px;width:50px;text-align:center;"+textColor+"'>"+discrepf+"</div>"
							 + "<div style='float:right;'><a href=\"javascript:;\" onclick=\"viewResults('"+rule_id+"','"+escapeHtml(title)+"');\" style='font-size:10px;text-decoration:underline;'>view</a></div>"
