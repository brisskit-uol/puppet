...
if (empty($file_fields)) exit("<button onclick=\"window.open('', '_self', '');window.close();window.top.close();open(window.location, '_self').close();self.close();\">{$lang['data_export_tool_160']}</button><br><br>{$lang['data_export_tool_159']}");

# Get doc_id's data for file fields (if user is in a DAG, limit to ONLY their DAG's records' files)
$docs = array();
$this_record = (isset($_GET['record']) ? trim(urldecode($_GET['record'])) : null);
		...
		foreach ($fields as $field_name=>$doc_id) {
			if (!empty($doc_id)) {
				...
				if ($longitudinal) {
					...
					$docs[$doc_id]['event_name'] = $Proj->eventInfo[$event]['name_ext'];
					$docs[$doc_id]['unique_event_name'] = $unique_events[$event];
				}
			}
		}
	}
}
if (empty($docs)) exit("<button onclick=\"window.open('', '_self', '');window.close();window.top.close();open(window.location, '_self').close();self.close();\">{$lang['data_export_tool_160']}</button><br><br>{$lang['data_export_tool_159']}");
...
if ($zip->open($target_zip, ZipArchive::CREATE) === TRUE) 
{
	...
	foreach ($docs as &$params) 
	{
		...
		$name = $params['record']
			  . ($longitudinal ? '_'.$params['unique_event_name'] : '')
			  . '_'.$params['field_name'].'.'.$params['file_extension'];
		...
		if ($longitudinal) {
			$this_row .= "<td>".RCView::escape($params['event_name'])."</td>";
		}
...
<html>
	...
	<div>
		...
		<table border=1 cellspacing=0>
			<thead><tr><th>{$lang['global_49']}</th>".($longitudinal ? "<th>{$lang['global_10']}</th>" : "")."<th>{$lang['design_484']}</th><th>{$lang['data_export_tool_168']}</th><th>{$lang['data_export_tool_169']}</th><th>{$lang['data_export_tool_170']}</th></tr></thead>
			<tbody>
			".implode('',$toc)."
			</tbody>
		</table>
...
// Logging
$log_descrip = "Download ZIP of uploaded files " . (isset($_GET['record']) ? "(single record)" : "(all records)");
$log_pk = (isset($_GET['record']) ? $_GET['record'] : $project_id);
$log_data_values = (isset($_GET['record']) ? "record = '{$_GET['record']}'" : "project_id = $project_id");
log_event("", "redcap_edocs_metadata", "MANAGE", $log_pk, $log_data_values, $log_descrip);
