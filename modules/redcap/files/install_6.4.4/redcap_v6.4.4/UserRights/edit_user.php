...
if (isset($_POST['submit-action'])) 
{
	...
	if ($user != '' && $_POST['role_name'] == '') {
		// User
		$context_msg_update = "<div class='userSaveMsg darkgreen' style='max-width:600px;text-align:center;'><img src='".APP_PATH_IMAGES."tick.png' class='imgfix'> ".$lang['global_17']." \"<b>$user</b>\" {$lang['rights_05']}</div>";
		$context_msg_insert = "<div class='userSaveMsg darkgreen' style='max-width:600px;text-align:center;'><img src='".APP_PATH_IMAGES."tick.png' class='imgfix'> ".$lang['global_17']." \"<b>$user</b>\" {$lang['rights_06']}</div>";
		$context_msg_delete = "<div class='userSaveMsg red' style='max-width:600px;text-align:center;'><img src='".APP_PATH_IMAGES."exclamation.png' class='imgfix'> ".$lang['global_17']." \"<b>$user</b>\" {$lang['rights_07']}</div>";
	} else {
		// Role
		if ($user == '0') {
			// New role
			...
			if ($submit_action == "edit_role" && $_POST['role_name_edit'] != $roles[$user]['role_name']) {
				log_event('',"redcap_user_rights","update",$user,"role = '$role_name',\nold role = '{$roles[$user]['role_name']}'","Rename role");
			}
		} else {
			$role_name = $roles[$user]['role_name'];
		}
		$context_msg_update = "<div class='userSaveMsg darkgreen' style='max-width:600px;text-align:center;'><img src='".APP_PATH_IMAGES."tick.png' class='imgfix'> ".$lang['global_115']." \"<b>$role_name</b>\" {$lang['rights_05']}</div>";
		$context_msg_insert = "<div class='userSaveMsg darkgreen' style='max-width:600px;text-align:center;'><img src='".APP_PATH_IMAGES."tick.png' class='imgfix'> ".$lang['global_115']." \"<b>$role_name</b>\" {$lang['rights_06']}</div>";
		$context_msg_delete = "<div class='userSaveMsg red' style='max-width:600px;text-align:center;'><img src='".APP_PATH_IMAGES."cross.png' class='imgfix'> ".$lang['global_115']." \"<b>$role_name</b>\" {$lang['rights_07']}</div>";
	}
	...
	// Copy role
		...
		if ($q) {
			...
			// Logging
			log_event($sql,"redcap_user_rights","insert",$user,"role = '$role_name'","Copy role");
		}
	...
	// Edit existing role
		...
		if (db_query($sql)) {
			...
			//Logging
			log_event($sql,"redcap_user_rights","update",$user,"role = '$role_name'","Edit role");
		}
	...
	// Edit existing user
		...
		if (db_query($sql)) {
			...
			$context_msg = $context_msg_update;
			//Logging
			log_event($sql,"redcap_user_rights","update",$user,"user = '".prep($user)."'","Edit user");
		}
	...
	// Add new role
		...
		if (db_query($sql)) {			
			...
			$context_msg = $context_msg_insert;
			// Logging
			log_event($sql,"redcap_user_rights","insert",$user,"role = '$role_name'","Add role");	
		}
	...
	// Add new user
		...
		if (db_query($sql)) {			
			...
			$context_msg = $context_msg_insert;
			// Logging
			log_event($sql,"redcap_user_rights","insert",$user,"user = '".prep($user)."'","Add user");	
		}
...
if (isset($_POST['username']) && $_POST['username'] != '' && $_POST['role_id'] == '') {
	...
	$isRole = true;
	$role_id = '0';
	$role_name = html_entity_decode($_POST['username'], ENT_QUOTES);
...
if (!$new_user) 
{
	...
	$deleteUserJs = "var delUserAction = function(){
					...
					simpleDialog('".cleanHtml($isRole ? $lang['rights_192'] : 
						(($user == USERID ? $lang['rights_194'] : $lang['rights_193']).(!MobileApp::userHasInitializedProjectInApp(USERID, PROJECT_ID) ? '' : 
	...
	if ($isRole) {
		...
		$context_msg =  RCView::img(array('src'=>'user_edit2.png', 'style'=>'vertical-align:middle;')) . 
						RCView::span(array('style'=>'vertical-align:middle;'), " {$lang['rights_09']} \"<b>$user</b>\"");
...
else 
{
	if ($isRole) {
		...
		$context_msg =  RCView::img(array('src'=>'user_add2.png', 'style'=>'vertical-align:middle;')) . 
						RCView::span(array('style'=>'vertical-align:middle;'), " {$lang['rights_11']} \"<b>$user</b>\"");
	...
	if ($chbx_email_newuser != "") {
		$chbx_email_newuser =  "<div style='margin:20px 0 0;position: relative;top:6px;z-index:106;color:#505050;width:160px;font-weight:bold;font-family:Verdana,Arial;font-size:11px;text-align:center;background:#eee;padding:2px;border:1px solid #bbb;border-bottom-width: 0px;'>
								...
								<div style='position: relative;border:1px solid #bbb;background:#eee;padding:10px 14px;'>
									<img src='".APP_PATH_IMAGES."email.png' class='imgfix'>&nbsp;&nbsp;{$lang['rights_112']}
									&nbsp;<input type='checkbox' name='notify_email' checked class='imgfix2'>
								</div>";
	}
}
	...
	if (!$new_user) {
		print  "<tr>
					<td valign='top' colspan='2' style='padding-bottom:5px;'>
						<img src='".APP_PATH_IMAGES."vcard.png'  class='imgfix'>
						&nbsp;&nbsp;{$lang['rights_199']}
...
print  "<tr>
			<td valign='top' colspan='2' style='border-top:1px solid #888;padding-top:4px;color:#800000;font-size:11px;'>
				{$lang['rights_299']}
			</td>
		</tr>";
...
//Data Access Groups
print "<tr><td valign='top' style='padding-bottom:10px;'><img src='".APP_PATH_IMAGES."group.png' class='imgfix'>&nbsp;&nbsp;{$lang['global_22']}</td><td valign='top' style='padding-top:2px;'> <input type='checkbox' name='data_access_groups' "; 
...
print "<tr><td valign='top' style='width:180px;'>
			<img src='".APP_PATH_IMAGES."layout_down_arrow.gif' class='imgfix'>&nbsp;&nbsp;{$lang['data_export_tool_186']}
...
print "<tr>
		<td valign='top'>
			<img src='".APP_PATH_IMAGES."layout.png' class='imgfix'>&nbsp;&nbsp;{$lang['rights_302']}
...
if ($enable_plotting > 0) {
	print "<tr><td valign='top'><img src='".APP_PATH_IMAGES."chart_bar.png' class='imgfix'>&nbsp;&nbsp;{$lang['report_builder_78']}</td><td valign='top' style='padding-top:2px;'> <input type='checkbox' name='graphical' "; 
...
{
	print "<tr><td valign='top'><img src='".APP_PATH_IMAGES."survey_participants.gif' class='imgfix' style='left:-4px;'>".$lang['app_22']."</td><td valign='top' style='padding-top:2px;'> <input type='checkbox' name='participants' "; 
...
print "<tr><td valign='top'><img src='".APP_PATH_IMAGES."table_row_insert.png'  class='imgfix'>&nbsp;&nbsp;{$lang['app_01']} </td><td style='padding-top:2px;' valign='top'> <input type='checkbox' name='data_import_tool' ";	if ($data_import_tool == 1) print "checked";
print "> </td></tr>
...
if ($data_resolution_enabled == '2') {
	...
	print "<tr><td valign='top' style='width:180px;'>
				<img src='".APP_PATH_IMAGES."balloon_tick.gif'  class='imgfix'>&nbsp;&nbsp;{$lang['dataqueries_137']} 
				<div style='padding:0px 0 0px 24px;font-size:11px;color:#777;font-family:tahoma;'>
					<a href='javascript:;' style='font-family:tahoma;font-size:10px;text-decoration:underline;' onclick=\"
...
// Mobile App
if (isDev(true) && $mobile_app_enabled) {
	...
	print  "<tr>
				<td valign='top' colspan='2' style='border-top:1px solid #888;padding:4px 0 2px;color:#800000;font-size:11px;'>
					{$lang['rights_309']}
				</td>
			</tr>
			<tr>
				<td valign='top'>
					<img src='".APP_PATH_IMAGES."redcap_app_icon.gif' class='imgfix'>&nbsp;&nbsp;{$lang['global_118']}
...
// DDP (only if enabled for whole system AND this project) - do NOT allow this for ROLES
if (!$isRole && is_object($DDP) && $DDP->isEnabledInSystem() && $DDP->isEnabledInProject()) 
{
	?>
	<tr>
		<td valign="top">
			<div style="margin-left:1.8em;text-indent:-1.8em;"> 
				<img src="<?php echo APP_PATH_IMAGES ?>databases_arrow.png" class="imgfix">&nbsp;&nbsp;<?php echo $lang['ws_51'] . " " . $DDP->getSourceSystemName() ?>
			</div>
		</td>
		<td valign="top" style="padding-top:2px;"> 
			<div style="margin-left:1.8em;text-indent:-1.8em;">
...
print "<div align='left' style='width:350px;'>
		...
		<tr>
			...
			<td valign='top' style='font-size:10px;text-align:left;width:205px;'>
				<div style='float:left;padding:2px 5px;white-space:normal;width:35px;'>{$lang['rights_47']}</div>
...
print "</td></tr>
	...
	<input type='hidden' name='user' value='".cleanHtml($isRole ? $role_id : $user)."'>
