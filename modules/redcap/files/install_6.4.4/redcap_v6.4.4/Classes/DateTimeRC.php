...
class DateTimeRC
{
	...
	{
		global $default_datetime_format, $datetime_formats, $default_datetime_format_system;
		// Return system format. Use value from config table, if valid.
		return (!in_array($default_datetime_format, $datetime_formats)) ? $default_datetime_format_system : $default_datetime_format;
	}
	 ...
	 */
	public static function format_user_datetime($val, $original_format, $return_format=null, $force24hour=false)
	{
		...
		if (!in_array($original_format, $datetime_formats)) {
			// Not valid, set to default system format
			$original_format = $default_datetime_format_system;
		}
		...
		if (strlen($time_val) > 5) $time_val = substr($time_val, 0, 5);
		...
		if ($time_val != '' && $time_return_format == '12' && $time_original_format == '24') {
			$time_val = self::format_time($time_val);
		}
	...
	public static function format_ts_from_ymd($val) {
		return self::format_user_datetime($val, 'Y-M-D_24');
	}
	...
	{
		global $lang, $datetime_formats;
		$options = array();
		// Loop through all options
		foreach ($datetime_formats as $option) 
		{
			$val = $option;
