...
if ($_POST['action'] == 'view')
{
	if (isset($_POST['existing_record']) && !$_POST['existing_record']) {
		...
		$content = $dq->displayFieldDataResHistory($record, $event_id, $field, $_POST['rule_id']);
	}
	## Output JSON
	print json_encode(array('content'=>$content, 'title'=>$title));
}
...
elseif ($_POST['action'] == 'save')
{
	...
	if ($dr_status == 'CLOSED' && isset($_POST['upload_doc_id']) && is_numeric($_POST['upload_doc_id'])) 
	{
		// Delete from table (i.e. set delete field to NOW)
		$sql = "update redcap_edocs_metadata set delete_date = '".NOW."' where project_id = " . PROJECT_ID . "
				and delete_date is null and doc_id = '" . prep($_POST['upload_doc_id']) . "'";
		db_query($sql);
	}
	...
	$sql = "insert into redcap_data_quality_status (rule_id, non_rule, project_id, record, event_id, field_name, query_status, assigned_user_id)
			values (".checkNull($rule_id).", ".checkNull($non_rule).", " . PROJECT_ID . ", '" . prep($record) . "', 
			$event_id, " . checkNull($field) . ", ".checkNull($dr_status).", ".checkNull($_POST['assigned_user_id']).")
	...
	if (db_query($sql)) 
	{
		...
		if (db_query($sql)) {
			...
			## Output JSON
			print json_encode(array('res_id'=>$res_id, 'icon'=>APP_PATH_IMAGES.$icon, 
			...
			## Logging
			$logDataValues = json_encode(array('res_id'=>$res_id,'record'=>$record,'event_id'=>$event_id,
			...
			// Log it
			log_event($sql,"redcap_data_quality_resolutions","MANAGE",$record,$logDataValues,$drw_log);
		} else {
			// ERROR!
			exit('0');
		}
