...
// Page title
renderPageTitle("<img src='".APP_PATH_IMAGES."balloons.png' class='imgfix2'> {$lang['dataqueries_141']}");
...
$dq = new DataQuality();
print RCView::div(array('id'=>'field_comment_parent','style'=>'margin:20px 0 0;'),
		$dq->renderFieldCommentLog(urldecode(label_decode($_GET['record'])), $_GET['event_id'], 
			$_GET['field'], $_GET['group_id'], $_GET['user'], urldecode(label_decode($_GET['keyword'])))
	  );
...
function reloadFieldCommentLog(show_progress) {
	...
	var keyword = $('#choose_keyword').trigger('focus').val();
	keyword = trim(keyword);
	// Keyword cannot be single letter
	if (keyword.length == 1) {
		simpleDialog('<?php echo cleanHtml($lang['dataqueries_230']) ?>','<?php echo cleanHtml($lang['global_01']) ?>');
		return;
	}
...
function openFieldCommentLogSearchTips() {
	simpleDialog('<?php echo cleanHtml($lang['dataqueries_232'].' "'.implode('", "', $dq->FCL_keywords_ignore).'"'.$lang['period'].'<br><br>'.$lang['dataqueries_251']) ?>','<?php echo cleanHtml($lang['dataqueries_141'].$lang['colon'].' '.$lang['dataqueries_231']) ?>');
}
</script>
<?php
