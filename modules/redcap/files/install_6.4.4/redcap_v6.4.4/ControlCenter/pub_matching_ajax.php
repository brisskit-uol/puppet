...
if ($_GET['action'] == 'getSetupDisplay') {
	...
	$inputs = array(
			...
			array('label' => RCL::pubMatchingEmailFreq() . RCView::div(array('class'=>'reqlbl'), RCL::required()),
			...
			array('label' => RCL::pubMatchingEmailLimit() . RCView::div(array('class'=>'reqlbl'), RCL::required()),
			...
			array('label' => RCL::pubMatchingEmailSubject() . RCView::div(array('class'=>'reqlbl'), RCL::required()),
      ...
      array('label' => RCL::pubMatchingEmailText() . RCView::div(array('class'=>'reqlbl'), RCL::required()),
        ...
        'input' => RCView::textarea(array('id' => 'pubEmailText', 'class' => 'x-form-field notesbox', 'style' => 'width: 250px; height: 100px;'),
          RCView::escape(empty($pub_matching_email_text) ? RCL::pubEmailIntro() : $pub_matching_email_text))),
			array('label' => RCL::pubInstLabel() . RCView::div(array('class'=>'cc_info'), $lang['control_center_4434']) .
				RCView::div(array('class'=>'reqlbl'), RCL::required()),
	...
	if (count($groups) === 0) {
		...
		$ajaxData = RCView::p(array(), RCL::pubTodoHelp()) .
					RCView::div(array('class'=>'blue','style'=>'margin-top:20px;padding:12px 10px 10px;width: 540px;'),
						RCView::img(array('src'=>'pencil.png','class'=>'imgfix')) . 
...
elseif ($_POST['action'] == 'savePubSettings') {
  ...
  $pubEmailText = empty($_POST['pub_email_text']) ? '' : trim($_POST['pub_email_text']);
	...
	foreach ($fieldToVal as $field => $val) {
		...
		if (count($sqlArr) === 1) {
			if (db_affected_rows()) RCLog::sysConfig($sqlArr[0]);
			$successes++;
		}
	}
...
elseif ($_GET['action'] == 'getPIEmailsDisplay') {
	...
	$ajaxData .= RCView::div(array('class' => 'redcapSimple'),
		...
		RCView::br() . RCView::br() .
		PubMedRedcap::getPIEmailBody('Watson', 3, array('REDCap', 'Example Program'), md5(rand())));
