...
if (!$isAjax) {
	...
	?>
	<style type="text/css">
	div#outer { width: 1200px; }
	div#mysql_dashboard { max-width:940px;width:940px; }
	</style>
	...
	function kill_query(mysql_id) {
		...
		simpleDialog(null,null,'kill_query_dialog',350,null,'<?php print cleanHtml($lang['global_53']) ?>',function(){
			kill = mysql_id;
			reload_mysql_dashboard();
		},'<?php print cleanHtml($lang['control_center_4460']) ?>');
	}
...
if ($allow_kill_mysql_process && isset($_GET['kill']) && is_numeric($_GET['kill'])) 
{
	...
	if (db_query("KILL $killed_process_id")) {
		$killed_process_msg = "<br><br><img src='".APP_PATH_IMAGES."tick.png' class='imgfix'> <span style='font-weight:bold;color:green;'>{$lang['control_center_4467']} #$killed_process_id</span>";
	} else {
		$killed_process_msg = "<br><br><img src='".APP_PATH_IMAGES."delete.png' class='imgfix'> <span style='font-weight:bold;color:#E00000;'>{$lang['control_center_4468']} #$killed_process_id</span>";
...
$link = "<a style='font-weight:normal;text-decoration:underline;' href='javascript:;' 
			...
			. ($_GET['view'] == "full" ? $lang['control_center_4463'] : $lang['control_center_4462']) 
	  . "</a>";
$reload_select =   "<select id='reload_dropdown' style='font-size:11px;' onchange=\"reload = this.value; reload_mysql_dashboard();\">
						<option value=''>- {$lang['control_center_4470']} -</option>
						<option value='2'  ".($_GET['reload'] == '2'  ? 'selected' : '').">{$lang['ws_91']} 2 {$lang['control_center_4469']}</option>
						<option value='5'  ".($_GET['reload'] == '5'  ? 'selected' : '').">{$lang['ws_91']} 5 {$lang['control_center_4469']}</option>
						<option value='10' ".($_GET['reload'] == '10' ? 'selected' : '').">{$lang['ws_91']} 10 {$lang['control_center_4469']}</option>
						<option value='30' ".($_GET['reload'] == '30' ? 'selected' : '').">{$lang['ws_91']} 30 {$lang['control_center_4469']}</option>
						<option value='60' ".($_GET['reload'] == '60' ? 'selected' : '').">{$lang['ws_91']} 60 {$lang['control_center_4469']}</option>
					</select>";
...
echo '<tr style="font-weight: bold;">
		<td valign="top" style="border-bottom:1px solid #ccc;background-color:#e5e5e5;padding:5px 10px;">
			<div style="float:left;margin-top:5px;margin-left:5px;">
				<span style="font-weight:bold;font-size:16px;">'.RCView::img(array('src'=>'database_table.png', 'class'=>'imgfix1')) . $lang['control_center_4457'].'</span>
				...
				<img id="mysql_dash_progress" src="'.APP_PATH_IMAGES.'progress_circle.gif" class="imgfix2" style="visibility:hidden;">
				<br><br>
				'."$link &nbsp; &nbsp; $reload_select".
				'<button id="stop_reload_btn" onclick="stop_reload();" style="margin-left:10px;font-size:11px;color:#800000;'.($_GET['reload'] == '' ? 'visibility:hidden;' : '').'">'.$lang['control_center_4483'].'</button>'.
				$killed_process_msg.'
			</div>
...
echo '<tr style="font-weight: bold;">
		...
		<td style="background-color:#fff;border-right:1px solid #eee;padding:5px;text-align:center;width:50px;font-size:11px;">
			'.$lang['control_center_4476'].'<div style="font-weight:normal;">'.$lang['control_center_4482'].'</div></td>
		<td style="background-color:#fff;border-right:1px solid #eee;padding:5px;text-align:center;width:50px;font-size:11px;">
			'.$lang['control_center_4477'].'<div style="font-weight:normal;">'.$lang['control_center_4482'].'</div></td>
		...
		<td style="background-color:#fff;padding:5px;width:340px;">'.$lang['control_center_4479'].'</td>
	  </tr>';
...
foreach ($processes as $row) 
{
	...
	echo   '<tr bgcolor="'.$bgcolor.'">
				...
				<td valign="top" class="wrap" style="width:40px;padding:3px 5px;color:#777;font-size:11px;">'.$row["State"].'</td>
