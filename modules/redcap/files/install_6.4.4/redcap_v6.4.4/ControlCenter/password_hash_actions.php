...
if (isset($_POST['action']) && $_POST['action'] == 'reminder')
{
	...
	if (db_query($sql)) {
		log_event($sql,"redcap_crons","MANAGE","","","Email all users with weaker password hash");
		print '1';
	}
}
...
elseif (isset($_POST['action']) && $_POST['action'] == 'suspend')
{
	...
	if (db_query($sql)) {
		log_event($sql,"redcap_user_information","MANAGE","","","Suspend all users with weaker password hash");
		print '1';
	}
