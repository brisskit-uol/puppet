...
if (isset($_GET['logged_events'])) {
	...
	$string = User::number_format_user($logged_events_total) . "|" .
	          User::number_format_user($logged_events_30min) . "|" .
...
if ($realtime_webservice_global_enabled) {
	...
	$sql = "select count(1) from redcap_projects where realtime_webservice_enabled = 1 and project_id not in (".$Stats->getIgnoredProjectIds().")";
	$total_ddp = db_result(db_query($sql), 0);
	...
	$sql = "select count(distinct(concat(p.project_id, '-', r.record))) 
			...
			where p.realtime_webservice_enabled = 1 and p.project_id = r.project_id and r.mr_id = d.mr_id 
			and p.project_id not in (".$Stats->getIgnoredProjectIds().")";
	$total_ddp_records_imported = db_result(db_query($sql), 0);	
	// Get count of all data points that have been imported from source system in DDP-enabled projects
	$sql = "select count(1) from redcap_projects p, redcap_ddp_records r, redcap_ddp_records_data d 
			where p.realtime_webservice_enabled = 1 and p.project_id = r.project_id and r.mr_id = d.mr_id
			and p.project_id not in (".$Stats->getIgnoredProjectIds().")";
...
$q = db_query("select count(1) from redcap_projects where is_child_of is not null and is_child_of != ''
...
if ($realtime_webservice_global_enabled) {
	$row_data[] = array("$indent1 {$lang['ws_187']}", User::number_format_user($total_ddp_records_imported));
	$row_data[] = array("$indent1 {$lang['ws_188']}", User::number_format_user($total_ddp_values_imported));
}
// Modules
...
$row_data[] = array("$indent1 {$lang['dashboard_64']}", User::number_format_user($parent_child_linkings));
