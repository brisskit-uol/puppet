...
if ($_GET['action'] == 'tokensByUser') {
	...
	$widths = $allUserMode ? array(70, 290, 50, 40, 106) : array(360, 50, 40, 106);
...
elseif ($_GET['action'] == 'tokensByProj') {
	...
	$widths = $allProjMode ? array(290, 70, 50, 40, 106) : array(360, 50, 40, 106);
	$ajaxData = RCView::simpleGrid($rows, $widths);
}
...
elseif ($_GET['action'] == 'getAPIRights') {
	...
	// Mobile App checkbox
	if (isDev()) {
		$attrs = array('name' => 'mobile_app', 'id' => 'mobile_app','class'=>'imgfix2');
	...
	$h .= 	RCView::div(array('style'=>'vertical-align:top;'),
				...
				RCView::div(array('class'=>'chklist','style'=>'margin:4px 0 10px;float:right;padding:10px;'), 
					RCView::checkbox(array('name' => 'api_send_email', 'id' => 'api_send_email', $notifyUserCheckbox => $notifyUserCheckbox,'class'=>'imgfix2')) . RCView::SP . RCView::SP . 
...
elseif ($_GET['action'] == 'reassignToken') {
	...
	elseif (empty($_GET['api_pid']) || !is_numeric($_GET['api_pid'])) {
		...
		if (isset($_GET['showDropDownOnly'])) {		
			...
			$sql = "update redcap_user_rights set api_token = null where username = '".prep($_GET['api_username'])."'
					and project_id = {$_GET['api_pid']}";
			db_query($sql);
			// Reassign token to new user
			$sql = "update redcap_user_rights set api_token = '".prep($token)."' where username = '".prep($_GET['new_user'])."'
					and project_id = {$_GET['api_pid']}";
			if (!db_query($sql)) {
				$ajaxData = RCView::errorBox($lang['control_center_261'] . ' - ' . $lang['control_center_259'], 'dialogAJAXId');
			}
			else {
				log_event("", "redcap_user_rights", "MANAGE", $_GET['api_username'], "user = '" . $_GET['api_username'] . "'", "Reassign API token to other user");
				$userInfo = $db->getUserInfoByUsername($_GET['api_username']);
				$projInfo = $db->getProject($_GET['api_pid']);
