...
$participant_id = getParticipantIdFromHash($hash);
...
if ($hasShortCode) {
	...
	// Output JSON
	exit(json_encode(array('code'=>$code, 'expiration'=>$expiration)));
}
...
$c = RCView::div(array('style'=>'font-size:'.($isAjax ? '14px' : '16px').';color:#800000;'), 
	...
	RCView::table(array('cellspacing'=>0, 'style'=>'table-layout:fixed;border-top:1px solid #ccc;padding-top:10px;width:100%;'), 
		RCView::tr(array(), 
			RCView::td(array('valign'=>'top', 'style'=>'padding-right:20px;'),
				## SURVEY ACCESS CODE
				(!gd2_enabled() ? '' :
				...
				(!$isAjax ? '' :
					...
					RCView::div(array('style'=>''),  
						...
						RCView::div(array('id'=>'gen_short_access_code_div', 'style'=>'margin-bottom:15px;'), 
							RCView::button(array('class'=>'jqbuttonmed', 'style'=>'font-size:14px;font-family:arial;color:#333;', 'onclick'=>"$(this).button('disable');getAccessCode('$hash',1);"),
								$lang['survey_653']
							) .
			...
			(!gd2_enabled() ? '' :
				RCView::td(array('id'=>'qrcode-info', 'valign'=>'top', 'style'=>'width:225px;padding-left:20px;border-left:1px solid #ccc;'),
					## QR CODE
					RCView::div(array('style'=>'color:#800000;font-weight:bold;font-size:15px;margin-bottom:10px;'), 
	...
	($isAjax ? '' :
		// Dialog 
		RCView::div(array('class'=>'hide_in_print', 'style'=>'text-align:center;margin:50px 0 0;'), 
			RCView::button(array('style'=>'color:#800000;font-size:15px;font-family:arial;padding:4px 8px;', 'onclick'=>"var currentWin = window.self;currentWin.close();"),
...
// Output JSON if AJAX
if ($isAjax) {
	print json_encode(array('content'=>$c, 'title'=>$t));
} else {
