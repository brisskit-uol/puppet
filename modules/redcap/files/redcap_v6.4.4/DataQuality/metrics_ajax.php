...
elseif ($chartname == 'num_open_queries_by_dag')
{
	...
	$dags = $Proj->getGroups();
	// Query tables
	$sql = "select d.value as group_id, count(1) as count 
			...
			and s.event_id = d.event_id and d.field_name = '__GROUPID__' 
			group by d.value order by count(*) desc";
	$q = db_query($sql);
	while ($row = db_fetch_assoc($q)) {
		// Get new max, if applicable
		if ($row['count'] > $max) $max = $row['count'];
...
elseif ($chartname == 'resolution_time_by_dag')
{
	...
	$sql = "select d.value as group_id, round(avg(z.sec_to_resolve)/86400,1) as avg_days_to_resolve 
			from (select x.record, TIMESTAMPDIFF(SECOND, x.min_ts, max(y.ts)) as sec_to_resolve 
			from (select s.status_id, s.record, min(r.ts) as min_ts from redcap_data_quality_status s, 
...
elseif ($chartname == 'response_time_by_dag')
{
	...
	$sql = "select d.value as group_id, round(avg(z.sec_to_respond)/86400,1) as avg_days_to_respond 
			from (select x.record, TIMESTAMPDIFF(SECOND, y.open_ts, x.response_ts) as sec_to_respond 
			from (select s.status_id, s.record, min(r.ts) as response_ts from redcap_data_quality_status s, 
...
// Return as JSON
print json_encode(array('min'=>(int)$min, 'max'=>(int)$max, 'data'=>$data));