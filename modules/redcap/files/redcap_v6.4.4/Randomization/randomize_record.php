...
elseif ($_POST['action'] == 'randomize')
{
	...
	$existing_record = ($_POST['existing_record'] == '1' && recordExists($record));
	...
	foreach ($fieldNamesEvents as $event_id=>$fields) {
		...
		list ($nothing1, $nothing2, $log_event_id) = saveRecord($record);
	...
	if (!empty($dq_errors)) 
	{
		...
		$sql = "delete from redcap_log_event where log_event_id in (".prep_implode($log_event_ids).")";
		db_query($sql);
		// Remove randomization field data from redcap_data (Part 1 of undoing randomization for this record)
		$sql = "delete from redcap_data where project_id = ".PROJECT_ID." and record = '".prep($record)."' 
				and event_id = '".prep($randAttr['targetEvent'])."' and field_name = '".prep($randAttr['targetField'])."'";
		db_query($sql);
		// Remove $aid from randomization_allocation table (Part 2 of undoing randomization for this record)
		$sql = "delete from redcap_randomization_allocation where aid = $aid";
		db_query($sql);
		...
		print 	RCView::div(array('class'=>"red", 'style'=>"margin:10px 0;"),
					...
					"<b>{$lang['global_01']}:</b> $table_pk_label \"<b>$record</b>\" {$lang['random_128']}"
				);
		// Display DQ rules pop-up message for Data Entry page
		$dq->displayViolationsSingleRecord($dq_errors, $record, $page_event_id);
