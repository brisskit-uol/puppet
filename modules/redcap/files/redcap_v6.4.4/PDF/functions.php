...
{	
	...
	$pdf->Cell(85,0,'');
	$pdf->Cell(0,0,'www.projectredcap.org',0,0,'L',false,'http://projectredcap.org');		
	//Set the REDCap logo
...
function removeSectionHeader($pdf, $heightPrevSH) 
{
...
function removeMatrixHeader($pdf, $heightPrevMH, $colsPrevMH)
{
	// Set number of lines of output that a section header takes up
	$mhLines = $colsPrevMH+3;
	// Get output for the current page
	$pageOutput = $pdf->pages[$pdf->PageNo()];
	// Split into separate lines
	$pageOutputLines = explode("\n", $pageOutput);
	if (count($pageOutputLines) > $mhLines) {
		// Remove last X lines
		$pageOutputLines = array_slice($pageOutputLines, 0, $mhLines*-1);
		// Re-add the last line as blank line
		$pageOutputLines[] = '';
		// Add these changes to the $pdf object
		$pdf->pages[$pdf->PageNo()] = $pageOutput = implode("\n", $pageOutputLines);
		// Reset Y position
		if ($pdf->GetY() > $heightPrevMH) {
			$pdf->SetY($pdf->GetY() - $heightPrevMH);
		}
	}
	// Return object
	return $pdf;
 ...
 */
function renderPDF($metadata, $acknowledgement, $project_name = "", $Data = array()) 
{	
	...
	elseif ($project_encoding == 'chinese_utf8')  
	{
		// Chinese
		$pdf = new PDF_Unicode();
		if (USE_UTF8) {
			...
			$pdf->AddUniGBhwFont();                
			//embeded subfont
			//$pdf->AddFont('uGB','','SURSONG.TTF',true);
			//$pdf->AddFont('uGB','B','simhei.ttf',true);
			//$pdf->AddFont('uGB','I','SURSONG.TTF',true);
			//$pdf->AddFont('uGB','BI','SURSONG.TTF',true);
		}
	} 
	else 
	{
		...
		foreach (array_keys($event_array) as $event_id)
		{
			...
			if ($record != '') 
			{
				...
				$study_id_event = "$table_pk_label $pk_display_val" . 
								  (isset($extra_record_labels[$record]) ? " ".$extra_record_labels[$record] : "");
				if ($project_encoding == 'japanese_sjis') {
				...
				if (isset($surveys_enabled) && ($surveys_enabled)) 
				{
					...
					if (db_num_rows($q) > 0) {
						...
						if ($completion_time == "" && $first_submit_time != "") {
							// Partial
							$response_time_text = "{$lang['data_entry_101']} {$lang['data_entry_100']} ".DateTimeRC::format_ts_from_ymd($first_submit_time).".";
						} elseif ($completion_time != "") {
							// Complete
							$response_time_text = $lang['data_entry_100']." ".DateTimeRC::format_ts_from_ymd($completion_time).".";							
						}
					}
				}
			}
			...
			if ($record != '') 
			{
				...
				foreach ($this_metadata as $key=>$attr)
				{
					...
					{
						// SKIP DROP-DOWN OPTION LABELS (they cannot be dynamic piping receivers b/c they can't have HTML inside their options)
						if ($attr['element_type'] == 'select' && $this_attr_type == 'element_enum') continue;
						...
						if ($this_string != '') 
						{
							...
							$this_metadata[$key][$this_attr_type] = strip_tags(Piping::replaceVariablesInLabel($this_string, $record, $event_id, $piping_record_data));
						}
					}
				}	
			}
			...
			foreach ($this_metadata as $row) 
			{
				...
				if (isset($Data[$record][$event_id][$row['field_name']]) && $Data[$record][$event_id][$row['field_name']] == '') {
					unset($Data[$record][$event_id][$row['field_name']]);
				}
				...
				if ($record != '') {
					...
					{
						$pdf = removeMatrixHeader($pdf, $heightPrevMH, $colsPrevMH);
					}
				}
				...
				{
					// Check if all questions in previous section were hidden. Skip if this is field 2 on form 1 (field right after table_pk).
					if ($record != '' && $fieldsDisplayedInSection === 0 && $encounteredFirstSH && $row['field_order'] != '2') {
						$pdf = removeSectionHeader($pdf, $heightPrevSH);
					}
					...
					$row['element_preceding_header'] = strip_tags(br2nl(label_decode($row['element_preceding_header'])));
				...
				if ($record != '')
				{
					...
					if (!(isset($Data[$record][$event_id][$row['field_name']]) && !is_array($Data[$record][$event_id][$row['field_name']])
						&& $Data[$record][$event_id][$row['field_name']] != '')) 
					{
						...
						if (isset($branchingLogicValid[$row['field_name']])) {
							...
							if ($longitudinal) {
								$row['branching_logic'] = LogicTester::logicPrependEventName($row['branching_logic'], $Proj->getUniqueEventNames($event_id));
							}				
							// Use the piping record data array as the data set to check the logic against (it will have FULL data for each record)
							...
							if (!$displayField) {
								...
								if ($matrixGroupPosition == '1') {
									...
									$pdf = renderMatrixHeaderRow($pdf, $thisEnum, $page_width, $matrix_label_width);
									// Calculate the total height of this matrix header
									$heightPrevMH = $pdf->GetY() - $mhHeightBefore;
									$colsPrevMH = count($thisEnum);
								}	
				...
				// if ($matrixGroupPosition == '1') {
					// print "MATRIX HEADERS<br>";
				// }
				// print $row['element_label']."<br>";
				...
				if ($row['element_type'] == "yesno" || $row['element_type'] == "truefalse" || $row['element_type'] == "radio" || $row['element_type'] == "select" || $row['element_type'] == "advcheckbox" || $row['element_type'] == "checkbox" || $row['element_type'] == "sql") 
				{
					...
					if ($row['grid_name'] != '') {
						...
						$pdf = renderMatrixRow($pdf, $row['element_label'], $enum, $enumData, $row_height, $sigil_width, $page_width, $matrix_label_width, $bottom_of_page,$study_id_event,($row['element_type'] == "advcheckbox" || $row['element_type'] == "checkbox"));
					}
					// LV, LH, RH Alignment
					elseif ($row['custom_alignment'] == 'LV' || $row['custom_alignment'] == 'LH' || $row['custom_alignment'] == 'RH') 
					{
						...
						foreach ($enum as $this_code=>$line)
						{
							...
							if ($row['element_type'] == "advcheckbox" || $row['element_type'] == "checkbox") {
								...
								$pdf = Circle($pdf, $x[0]+1.5, $y[0]+1.5, 1.6);
								$crosslineoffset = 0.5;
							}
				...
				// Descriptive
					...
					if ($display_inline_image) {
						...
						if ($imgfile_path !== false) {
							...
							$pdf = new_page_check(($img_height/$y_units_per_line)+$counter, $pdf, $y_units_per_line, $bottom_of_page, $study_id_event);
							// If a survey and using custom question numbering, then render question number				
				...
				// Text, Notes, Calcs, and File Upload fields
					...
					if ($row['custom_alignment'] == 'LV' || $row['custom_alignment'] == 'LH') 
					{
						...
						if (isset($Data[$record][$event_id][$row['field_name']]) && $row['element_type'] == "file" && $row['element_validation_type'] == "signature") {	
							// Need about 6 lines worth of height for this image							
							$pdf = new_page_check(6, $pdf, $y_units_per_line, $bottom_of_page, $study_id_event);	
							// Get current position, so we can reset it back later
							$y = $pdf->GetY();
							$x = $pdf->GetX();
							// Copy file to temp directory
							$sigfile_path = Files::copyEdocToTemp($Data[$record][$event_id][$row['field_name']], true);
					...
					else
					{
						...
						if (isset($Data[$record][$event_id][$row['field_name']])) {
							//Has data
							$this_textv = $Data[$record][$event_id][$row['field_name']];
							if ($project_encoding == 'japanese_sjis') {
			...
			{		
				...
				if ($displayLocking)
				{
					...
					{			
						...
						$lock_string .= "on " . DateTimeRC::format_ts_from_ymd($form_locked['timestamp']);
						...
						if ($displayEsignature)
						{
							...
							if (db_num_rows($q)) 
							{
								...
								$lock_string = "E-signed by {$form_esigned['username']} ({$form_esigned['user_firstname']} "
											 . "{$form_esigned['user_lastname']}) on " . DateTimeRC::format_ts_from_ymd($form_esigned['timestamp']) 
											 . "\n" . $lock_string;					
							}
						}
	...
	// Output to file (or as string for Plugins only)
	if (defined("PLUGIN")) {
...
// Draw circle
function Circle($pdf, $x, $y, $r, $style='')
{
	return Ellipse($pdf, $x, $y, $r, $r, $style);
}
...
function renderMatrixRow($pdf,$label,$hdrs=array(),$enumData,$row_height,$sigil_width,$page_width,$matrix_label_width,$bottom_of_page,$study_id_event,$isCheckbox)
{
	...
	foreach ($data as $key=>$isChecked)
	{
		...
		if($i!=0) {
			...
			if ($isCheckbox) {
				...
				$pdf = Circle($pdf, $xboxpos+1.5, $y+1.5, 1.6);
				$crosslineoffset = 0.5;
			}
			// Positions of line 1
			$line1_x0 = $xboxpos;
