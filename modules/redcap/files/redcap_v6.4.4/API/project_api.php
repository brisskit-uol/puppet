...
if (SSL) {
	$instr .= RCView::p(array('class'=>'yellow', 'style'=>'margin:20px 0;'),
				RCView::img(array('src' => 'exclamation_orange.png','class'=>'imgfix2')) .
...
$tok .= RCView::div(array('style' => 'margin:10px 0 25px;'),
			RCView::div(array('style' => 'font-weight:bold;'), 
				RCView::img(array('src'=>'coin.png', 'class'=>'imgfix')) . 
...
if ($longitudinal && count($events) > 0) {
	...
	$e = RCView::div(array('style'=>'margin:20px 0;'), RCView::simpleGrid($eventRows, array(200, 200, 100)));
}
...
if (SUPER_USER) {
	$tabs = array('API/project_api.php'=>RCView::img(array('src'=>'coin.png', 'class'=>'imgfix')) . $lang['control_center_340'], 
				  'API/project_api.php?allUserTokens=1'=>RCView::img(array('src'=>'coins.png', 'class'=>'imgfix')) . $lang['control_center_341']);
	renderTabs($tabs);
}
