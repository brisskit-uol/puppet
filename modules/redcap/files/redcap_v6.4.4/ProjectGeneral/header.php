		...
		|| !$longitudinal) 
	{
		...
		if ($longitudinal && isset($_GET['event_id']) && is_numeric($_GET['event_id'])) 
		{
			$event_label = "<div style='padding:1px 0 5px;'>
								{$lang['bottom_23']}&nbsp;
								<span style='color:#800000;font-weight:bold;'>{$Proj->eventInfo[$_GET['event_id']]['name_ext']}</span>
							</div>";
		}
...
// Mobile app
$mobile_app_testers = array('salvucm', 'pearsosj', 'keilcd', 'sanforlb', 'fernanm', 'minorbl', 'harrispa', 'elliotvj', 'scottta1');
if ($status < 2 && $mobile_app_enabled && $api_enabled 
	&& ($user_rights['mobile_app'] || isDev(true) || (isVanderbilt() && in_array(strtolower(USERID), $mobile_app_testers))) )
{
