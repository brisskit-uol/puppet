...
if (isset($_GET['pid']) && is_numeric($_GET['pid'])) 
{
	...
	if ($last_logged_event == "") {
		...
		$sql = "select timestamp(ts) from redcap_log_event force index (user_project) 
		...
		if ($most_recent_event != "") {
			$sql = "update redcap_projects set last_logged_event = '".prep($most_recent_event)."' where project_id = " . PROJECT_ID;
			db_query($sql);
		}
	} else {
	...
	$sql = "select count(distinct(record)) from redcap_data 
			where project_id = " . PROJECT_ID . " and field_name = '$table_pk'";
	$num_records = User::number_format_user(db_result(db_query($sql),0));
	...
	if ($user_rights['group_id'] != "") 
	{
		...
		$num_records = "{$lang['data_entry_103']} ".User::number_format_user($num_records)." / {$lang['data_entry_104']} ".User::number_format_user($num_records_group);
	}
...
elseif (isset($_POST['pids']))
{
	...
	if ($_POST['type'] == 'records')
	{
		...
		$sql = "select m.project_id, count(distinct(d.record)) as count from redcap_data d, redcap_metadata m 
				...
				and m.field_order = 1 and m.project_id in (" . prep_implode($pids) . ") 
	...
	elseif ($_POST['type'] == 'fields')
	{
		...
		$sql = "select p.project_id, count(m.field_name) as num_fields, count(distinct(m.form_name)) as num_instruments, 
				...
				where p.project_id in (" . prep_implode($pids) . ") and m.project_id = p.project_id and m.element_type != 'descriptive' 
				group by m.project_id order by m.project_id";
		$q = db_query($sql);
		while ($row = db_fetch_assoc($q)) 
		{
