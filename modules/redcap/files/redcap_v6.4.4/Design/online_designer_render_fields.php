...
if (count($metadata) > 0) {
	...
	foreach ($metadata as $row)
	{
		...
		$hasSHtag = false;
		if ($element_preceding_header && ($field_name != $table_pk) && (!$_GET['edit_question'] || $_GET['section_header'])) 
		{
			...
			if ($isMatrixField) {
				...
				$shIcons = "<div class=\'frmedit\' style=\'padding-bottom:4px;\'>"
						 . "<a href=\'javascript:;\' onclick=\'openAddQuesForm(\"$field_name\",\"$element_type\",1,0);\'><img src=\'".APP_PATH_IMAGES."pencil.png\' class=\'imgfix\' ignore=\'Yes\'></a> "
						 . "<a href=\'javascript:;\' onclick=\'deleteField(\"$field_name\",1);\'><img src=\'".APP_PATH_IMAGES."cross.png\' class=\'imgfix\' ignore=\'Yes\'></a> "
						 . "</div>";
			}
			$element_preceding_header = nl2br(cleanLabel(filter_tags(label_decode($element_preceding_header))));
