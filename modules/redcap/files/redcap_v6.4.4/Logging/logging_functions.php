...
function renderLogRow($row, $html_output=true)
{
		...
		switch ($row['event']) {
			...
			case 'UPDATE':
				...
				if ($row['object_type'] == APP_NAME."_data" || $row['object_type'] == "redcap_data") 
				{
					...
					if (strpos($row['description'], " (import)") !== false) {
						$event .= " (import)";
					}
					elseif (strpos($row['description'], " (API)") !== false) {
						$event .= " (API)";
					}
					elseif (strpos($row['description'], " (Auto calculation)") !== false) {
					...
					{				
						$event  = "<font color=#000066>{$lang['reporting_25']}";
						if ($row['page'] == "DataImport/index.php") {
							$event .= " (import)";
						} elseif (strpos($row['description'], " (API)") !== false) {
							$event .= " (API)";
						}
	...
	if ($longitudinal && $row['legacy'] == '0' 
		 && ((($row['object_type'] == "redcap_data" || $row['object_type'] == "") && in_array($row['event'], $dataEvents))
			|| ($row['event'] == 'MANAGE' && $row['object_type'] == "redcap_edocs_metadata"))
		)
	{
