...
class DataQuality
{
	...
	private function executePredefinedRule($rule_id, $rule_record=null, $dag_discrep)
	{
		...
		switch ($rule_id)
		{
			...
			case 'pd-6':
				...
				if ($rule_record != '') $sql .= " and record = '".prep($rule_record)."'";
				$sql .= " order by abs(record), record, event_id";
				$q = db_query($sql);
				while ($row = db_fetch_assoc($q))
				{
			...
			case 'pd-5':
				...
				$sql = "select record, event_id, field_name, value from redcap_data where project_id = ".PROJECT_ID." and record != ''
						and field_name in ('" . implode("', '", $numericalFields) . "') $group_sql
						and event_id in ($eventIdsSql)";
				if ($rule_record != '') $sql .= " and record = '".prep($rule_record)."'";
				$sql .= " order by abs(record), record, event_id";
				$q = db_query($sql);
			...
			case 'pd-7':
				...
				$sql = "select record, event_id, field_name, value from redcap_data where project_id = " . PROJECT_ID . " 
						...
						$group_sql and event_id in ($eventIdsSql)";
				if ($rule_record != '') $sql .= " and record = '".prep($rule_record)."'";
				$sql .= " order by abs(record), record, event_id";
				$q = db_query($sql);
				while ($row = db_fetch_assoc($q))
			...
			case 'pd-9':				
				...
				$recordData = array();
				$sql = "select record, event_id, field_name, value from redcap_data where project_id = ".PROJECT_ID." and record != ''
						and field_name in ('" . implode("', '", array_keys($fields)) . "') $group_sql
						and event_id in ($eventIdsSql)";
				if ($rule_record != '') $sql .= " and record = '".prep($rule_record)."'";
				$sql .= " order by abs(record), record, event_id";
				$q = db_query($sql);
			...
			case 'pd-10':
				...
				if (isset($_POST['action']) && $_POST['action'] == 'fixCalcs') {
					...
					$this->valuesFixed = Calculate::saveCalcFields($record_array);
				} else {
					// Perform calculations on ALL calc fields over ALL records, and return those that are incorrect
					$calcFieldData = Calculate::calculateMultipleFields($record_array, array(), true);
						...
						foreach ($calcFieldData as $record=>&$this_record_data) {
							foreach ($this_record_data as $event_id=>&$this_event_data) {
								...
								foreach ($this_event_data as $field=>$attr) {
									...
									$data_display = "$field = $value_html<div class='wrap' style='color:#800000;;'>({$lang['dataqueries_291']} \"{$attr['calc']}\")</div>";
									// Is this record-event excluded for this rule?
									$excludeRecEvt = (isset($excluded[$record][$event_id][$field]) ? 1 : 0);
									// Save result
									$this->saveLogicCheckResults($rule_id, $record, $event_id, '', '', $data_display, $excludeRecEvt, 0, $field);
	...
	public function executeRule($rule_id, $rule_record=null)
	{
		...
		$excluded = array();
		$statuses = array();
		$sql = "select record, event_id, exclude, status, query_status 
		...
		while ($row = db_fetch_assoc($q))
		{
			...
			if (($data_resolution_enabled != '2' && $row['exclude']) || ($data_resolution_enabled == '2' 
				&& ($row['query_status'] == 'VERIFIED' || $row['query_status'] == 'CLOSED'))) {
		...
		$sql = "select record, event_id, field_name, value from redcap_data where project_id = " . PROJECT_ID 
			 . " and (field_name in ('" . implode("', '", $fields) . "') or (" . $checkbox_sql . ")) and record != '' $group_sql";
		if ($rule_record != '') $sql .= " and record = '".prep($rule_record)."'";
		$sql .= " order by abs(record), record, event_id";
		$q = db_query($sql);
		$prevEvent = 0;
			...
			if ($first || $row['record'] !== $prevRecord) {
				...
				if (!$first) {
					$this->applyRuleLogicToAllEvents($funcName, $record_data, $rule_attr,
						$prevRecord, $excluded, $statuses, $rule_id, $dag_discrep, $ruleContainsOneField);
				}
	...
	private function applyRuleLogicToAllEvents($funcName, &$recordData,
		$rule_attr, $record, &$excluded, &$statuses, $rule_id, &$dag_discrep, $ruleContainsOneField='')
	{
		...
		foreach ($recordData as $eventId => $foo) {
			...
			if ($logicCheckResult !== 1)
			{
				...
				$this->saveLogicCheckResults($rule_id, $record, $eventId, $rule_attr['logic'],
					$this->logicFuncToCode[$funcName], $data_display, $excludeRecEvt, $status, $ruleContainsOneField);
			}
		}
	}
	...
	private function loadRulesTable()
	{
		...
		foreach ($this->getRules() as $rule_id=>$rule_attr)
		{
			...
			$rulesTableData[$counter][] = !is_numeric($rule_id) ? "" :
				"<div class='editrte' style='text-align:center;padding:2px 0;' id='rulerte_{$rule_id}'>
					<img src='".APP_PATH_IMAGES."$rteImage' class='imgfix1'>
					<input type='checkbox' id='rulerte_newvalue_{$rule_id}' style='display:none;' class='imgfix2' disabled $rteChecked>
		...
		if ($user_rights['data_quality_design']) {
			...
			$rulesTableData[$counter][] = "<div class='newlogic'>
											...
											<div style='padding:5px 0 0;'><a href='javascript:;' style='font-size:10px;' onclick=\"helpPopup('logic_functions')\">{$lang['dataqueries_79']}</a></div>
										 </div>";
	...
	{	
		...
		$dags = $Proj->getGroups();
		// Set the table width
		$width = 817 + ((!empty($dags) && $user_rights['group_id'] == "") ? count($dags)*62 : 0);
		...
		if ($user_rights['data_quality_execute']) 
		{
			...
			$title .=  "<div style='float:right;padding:2px 0 0;text-align:right;font-weight:bold;'>
							...
							<button class='execRuleBtn' onclick=\"$('#dqRuleRecord').prop('disabled',true);$('.execRuleBtn').prop('disabled',true);$('#dq_results').html('');preExecuteRulesAjax(rule_ids,0);\">{$lang['dashboard_12']}</button>
							<button class='execRuleBtn' onclick=\"$('#dqRuleRecord').prop('disabled',true);$('.execRuleBtn').prop('disabled',true);$('#dq_results').html('');preExecuteRulesAjax(rule_ids_excludeAB,0);\">{$lang['dataqueries_116']}</button>
							<button class='execRuleBtn' onclick=\"$('#dqRuleRecord').prop('disabled',true);$('.execRuleBtn').prop('disabled',true);$('#dq_results').html('');preExecuteRulesAjax(rule_ids_user_defined,0);\">{$lang['dataqueries_117']}</button>
							&nbsp;<button id='clearBtn' disabled onclick=\"window.location.href=app_path_webroot+page+'?pid='+pid;\">{$lang['dataqueries_86']}</button>
						</div>
						<br><br>
						<div style='float:right;font-weight:normal;font-family:arial;color:#800000;'>
							{$lang['dataqueries_297']}
							<select id='dqRuleRecord' class='x-form-text x-form-field' style='max-width:300px;font-size:11px;margin-left:2px;padding-right:0;padding-top: 1px;height:19px;'>
								$recordList
							</select>
						</div>";
		}
				...
				if ($data_resolution_enabled == '2') {
					...
					if ($user_rights['data_quality_resolution'] == '0') {
						...
						$excludeAction = RCView::button(array('class'=>'jqbuttonmed', 'style'=>'font-size:11px;',
											'onclick'=>"dataResPopup('{$results['field_name']}',{$results['event_id']},'".cleanHtml($results['record'])."',1,'$rule_id');"),
											RCView::img(array('id'=>"dc-icon-{$rule_id}_{$results['field_name']}__{$results['record']}", 'src'=>$balloonIcon,'style'=>'vertical-align:middle;')) .
											RCView::span(array('style'=>'vertical-align:middle;font-family:arial;line-height:1.3;'),
	...
	public static function getExclusionsSingleRecord($record, $event_id)
	{
		global $data_resolution_enabled;
		// Query for exclusions and put in array
		$excluded = array();
		$sql = "select distinct rule_id, field_name from redcap_data_quality_status where project_id = " . PROJECT_ID . " 
	...
	public function displayViolationsSingleRecord($dq_error_ruleids=array(), $record, $event_id, $current_form=null, $show_excluded=0)
	{
		...
		foreach ($dq_rules_violated_fields as $rule_id=>$dq_rule_fields) 
		{
			...
			if ($data_resolution_enabled == '2') {
				...
				$drw_history = $this->getFieldDataResHistory($record, $event_id, '', $rule_id);
				...
				if ($user_rights['data_quality_resolution'] == '0') {
					...
					// Display button
					$excludeAction = RCView::button(array('class'=>'jqbuttonmed', 'style'=>'font-size:11px;',
										'onclick'=>"dataResPopup('$dataQueryField',$event_id,'".cleanHtml($record)."',1,'$rule_id');"),
				...
				## EXCLUDE / REMOVE EXCLUSION
				if ($isExcluded) {
					$excludeAction = RCView::a(array('href'=>'javascript:;', 'style'=>'font-size:10px;color:#800000;text-decoration:underline;', 'onclick'=>"excludeDQResult(this,'$rule_id',0,'".cleanHtml($record)."',$event_id,'');"), 
										$lang['dataqueries_88']
									);
				} else {
					$excludeAction = RCView::a(array('href'=>'javascript:;', 'style'=>'font-size:10px;text-decoration:underline;', 'onclick'=>"excludeDQResult(this,'$rule_id',1,'".cleanHtml($record)."',$event_id,'');"), 
										$lang['dataqueries_87']
									);
				}
			}
		...
		if (!empty($r))
		{
			...
			if ($current_form != null) {
				$sql = "select 1 from redcap_locking_data where project_id = " . PROJECT_ID . " and record = '" . prep($record) . "' 
						and event_id = $event_id and form_name = '" . prep($current_form) . "' limit 1";
				$q = db_query($sql);
				$formIsLocked = (db_num_rows($q) > 0);
				if ($formIsLocked) {
					$lockingMsg .= 	RCView::img(array('src'=>'lock.png','class'=>'imgfix2')) .
					...
					if ($user_rights['lock_record'] > 0) {
						$lockingMsg .= 	" " . $lang['data_entry_184'] .
										"<input id='unlockBtnDQ' style='vertical-align:middle;font-size:11px;' type='button' onclick=\"unlockForm('$(\'#unlockDQdiv\').hide()')\" value='".cleanHtml($lang['data_entry_182'])."'>";
					}
					// Wrap msg in div
	...
	public function saveExcludeForRule($rule_id=null, $record=null, $event_id=null, $status=null, $field_name='', $exclude=null)
	{
		global $lang, $Proj;
	...
	public static function fieldsWithDataResHistory($record, $event_id, $form)
	{
		...
		$sql = "select x.*, if (y.response is null or x.status='CLOSED', 0, 1) as responded from 
				...
				and s.status_id = r.status_id and m.project_id = s.project_id 
	...
	public function getFieldDataResHistory($record, $event_id, $field, $rule_id='')
	{
	...
	public function displayFieldDataResHistory($record, $event_id, $field, $rule_id='')
	{
		...
		$drw_history = $this->getFieldDataResHistory($record, $event_id, $field, $rule_id);
		...
		if ($data_resolution_enabled == '2')
		{
			...
			$queryStatusLabel = RCView::div('',
									...
									RCView::img(array('src'=>$currentStatusIcon,'class'=>'imgfix')) .
		...
		$h .= 	RCView::div(array('style'=>'margin:0 0 15px;'),
					...
					($data_resolution_enabled == '2' 
						? 	// DRW instructxions
							RCView::div(array('style'=>'text-align:right;margin:0 10px 5px;'),
								RCView::img(array('class'=>'imgfix','src'=>'video_small.png')) .
					...
					RCView::div(array('style'=>'margin-top:10px;'), 
						...
						($field == '' 
							...
							: RCView::a(array('href'=>APP_PATH_WEBROOT."DataEntry/index.php?pid=".PROJECT_ID."&event_id=$event_id&id=".
		...
		$h .=
			...
			((	$data_resolution_enabled == '1'
				...
				// Render new form
				? self::renderFieldDataResHistoryNewForm($record, $event_id, $field, $rule_id, $prevUserAttr)
				...
				:	(($data_resolution_enabled == '2' && $prevUserAttr['current_query_status'] != 'CLOSED')
						? 	RCView::div(array('class'=>'yellow', 'style'=>'margin:20px 0;'), 
								RCView::img(array('src'=>'exclamation_frame.png','class'=>'imgfix')) .
					...
					RCView::td(array('class'=>'data nowrap', 'style'=>'border:1px solid #ddd;padding:3px 0;width:35px;text-align:center;'.$cellstyle), 
						RCView::div(array('style'=>'margin-bottom:3px;'), 
							RCView::a(array('href'=>'javascript:;', 'onclick'=>"editFieldComment({$attr['res_id']},'$form_name',1,0);"), 
								RCView::img(array('src'=>'pencil.png', 'class'=>'imgfix1', 'title'=>$lang['global_27']))
							)
						) .
						RCView::div(array('style'=>''), 
							RCView::a(array('href'=>'javascript:;', 'onclick'=>"deleteFieldComment({$attr['res_id']},'$form_name',1);"), 
								RCView::img(array('src'=>'cross.png', 'class'=>'imgfix1', 'title'=>$lang['design_170']))
							)
						)
					)
				) .
				// Date/time
				RCView::td(array('class'=>'data nowrap', 'style'=>'border:1px solid #ddd;padding:3px 8px;text-align:center;width:130px;'.$cellstyle), 
				...
				RCView::td(array('class'=>'data', 'style'=>'border:1px solid #ddd;padding:3px 8px;'.$cellstyle),
					...
					(!$userResponded ? '' :
						RCView::div(array('style'=>''), 
							...
							RCView::span(array('style'=>'color:#000088;'), self::getDataResolutionResponseChoices($attr['response']))
						)
					) .
	...
	public static function renderFieldDataResHistoryNewForm($record, $event_id, $field, $rule_id, $prevUserAttr=array())
	{
...
;
		...
		elseif ($data_resolution_enabled == '2' && $prevUserAttr['response_requested'])
		{
			...
			$td .=
				RCView::div(array('style'=>''),
					$radioRespondOption . RCView::span(array('style'=>'color:#000088;font-weight:bold;'), $lang['dataqueries_200']) . " &nbsp;" .
		...
		return 	RCView::table(array('id'=>'newDCHistory','class'=>'form_border','cellspacing'=>'0','style'=>'table-layout:fixed;width:100%;'), 
					RCView::tr('', 
						...
						RCView::td(array('id'=>'newDCnow','class'=>'data', 'style'=>'border:1px solid #ccc;padding:3px 8px;text-align:center;width:130px;'.$bgColor), 
				...
				RCView::div(array('style'=>'padding:15px 0 7px;text-align:right;font-size:13px;font-weight:bold;vertical-align:middle;'),
					...
					RCView::div(array('style'=>'float:right;'), 
						RCView::button(array('id'=>'dataResSavBtn', 'class'=>'jqbutton', 'style'=>'padding: 0.4em 0.8em !important;margin-right:3px;', 
							'onclick'=>"dataResolutionSave('".cleanHtml($field)."','".cleanHtml($event_id)."','".cleanHtml($record)."','".cleanHtml($rule_id)."');"), $saveBtn
						) 
					) .
					// "Saved!" msg
	...
	public static function renderDataResFileUploadDialog()
	{
		...
		return	RCView::div(array('class'=>'simpleDialog','style'=>'font-size:12px;display:none;', 'id'=>'drw_file_upload_popup','title'=>$lang['form_renderer_23']), 
					...
					RCView::form(array('id'=>'drw_upload_form', 'method'=>'post', 'target'=>'drw_upload_target', 'enctype'=>'multipart/form-data', 
						...
						RCView::input(array('type'=>'hidden','name'=>'redcap_csrf_token', 'value'=>getCsrfToken()))
					) .
	...
	public function renderTabs()
	{
		...
		if ($user_rights['data_quality_execute'] + $user_rights['data_quality_design'] > 0) {
			// Add DQ tab
			$tabs['DataQuality/index.php']   = RCView::img(array('src'=>'magnifier.png','class'=>'imgfix')) . $lang['dataqueries_193'];
		}
		if ($data_resolution_enabled == '2' && $user_rights['data_quality_resolution'] > 0) {
			...
			// Add DRW tabs
			$tabs['DataQuality/resolve.php'] = RCView::img(array('src'=>'balloons.png','class'=>'imgfix')) . $lang['dataqueries_148'] . $numOpenIssuesHtml;
			$tabs['DataQuality/metrics.php']  = RCView::img(array('src'=>'statistics.png','class'=>'imgfix')) . $lang['dataqueries_194'];
	...
	public function getDataIssuesByStatus($issueStatusType='OPEN')
	{
		...
		$sql = "select x.*, a.ts as ts_first, a.user_id as user_id_first, a.comment as comment_first, 
				...
				order by abs(x.record), x.record, x.event_id, x.field_name";
		$q = db_query($sql);
		while ($row = db_fetch_assoc($q)) 
		{
	...
	public function renderResolutionTable($issueStatusType='OPEN', $fieldRuleFilter='', $event_id='', $group_id='', $assigned_user_id='')
	{
		...
		foreach ($dataIssues as $status_id=>$attr)
		{
			...
			if ($attr['field_name'] == '') {
				$record_label = RCView::span(array('style'=>'font-size:12px;'), $attr['record']);
			} else {
				$record_label = RCView::a(array('href'=>APP_PATH_WEBROOT."DataEntry/index.php?pid=".PROJECT_ID."&event_id={$attr['event_id']}&id={$attr['record']}&page=".$Proj->metadata[$attr['field_name']]['form_name']."&fldfocus={$attr['field_name']}#{$attr['field_name']}-tr", 'style'=>'text-decoration:underline;'), $attr['record']);			
			}
			...
			$resData[] = array(
							RCView::button(array('class'=>'jqbuttonmed', 'style'=>'font-size:11px;',
								'onclick'=>"dataResPopup('{$attr['field_name']}',{$attr['event_id']},'".cleanHtml($attr['record'])."',1,'$rule_id');"),
	...
	public static function dataResolutionAutoDeverify($data=array())
	{
		...
		foreach ($status_ids as $status_id=>$attr) {
			...
			if (db_query($sql)) {
				...
				// Log it
				log_event($sql,"redcap_data_quality_resolutions","MANAGE",$record,$logDataValues,"De-verified data value");
				// Increment counter
				$num_deverified++;
			}
		}
	...
	public function getFieldCommentLog($record=null, $event_id=null, $field=null, $group_id=null)
	{
		...
		// Get field comments
		$sql = "select s.record, s.event_id, s.field_name, r.comment, r.ts, i.username 
				from redcap_data_quality_status s, redcap_data_quality_resolutions r 
		...
		while ($row = db_fetch_assoc($q)) 
		{
			...
			// Add values to array
			$comments[$row['record']][$row['event_id']][$row['field_name']][] 
	...
	public function renderFieldCommentLog($record=null, $event_id=null, $field=null, $group_id=null, $user=null, $keyword=null)
	{
				...
				foreach ($eattr as $this_field=>$fattr) {
					...
					foreach ($fattr as $attr) {
						...
						if (!isset($rows[$row_counter])) 
						{
							...
							$record_display = RCView::a(array('target'=>'_blank', 'href'=>APP_PATH_WEBROOT."DataEntry/index.php?pid=".PROJECT_ID."&event_id=$this_event_id&id=$this_record&page=".$Proj->metadata[$this_field]['form_name']."&fldfocus=$this_field#$this_field-tr", 'style'=>'text-decoration:underline;'), $this_record)
							...
							$rows[$row_counter] = array(
								RCView::button(array('class'=>'jqbuttonmed', 'style'=>'font-size:11px;',
									'onclick'=>"dataResPopup('$this_field',$this_event_id,'".cleanHtml($this_record)."',1,'');"),
		...
		$recordDropdown = RCView::select(array('id'=>'choose_record', 'style'=>'font-size:11px;'), $recordDropdownOptions, $record);
		...
		{		
			...
			$eventDropdown = RCView::span(array('style'=>'color:#777;margin:0 4px 0 2px;'), $lang['data_entry_67']) . 
							 RCView::select(array('id'=>'choose_event', 'style'=>'font-size:11px;', 
		...
		$fieldDropdown = RCView::select(array('id'=>'choose_field', 'style'=>'font-size:11px;', 
		...
		if (!empty($dags) && $user_rights['group_id'] == "")
		{
			...
			$dagDropdown = RCView::select(array('id'=>'choose_dag', 'style'=>'font-size:11px;', 
		...
		$userDropdown = RCView::select(array('id'=>'choose_user', 'style'=>'font-size:11px;', 
		...
		$tableTitle = RCView::div(array('style'=>'width:170px;font-size:13px;float:left;padding:5px 0 0 10px;'), 
						...
						RCView::div(array('style'=>'font-size:11px;float:left;font-weight:normal;padding-top:2px;'),
							...
							RCView::div(array('style'=>'margin-left:45px;'), 
								$keywordTextbox .
								RCView::a(array('href'=>'javascript:;','onclick'=>'openFieldCommentLogSearchTips();','style'=>'margin-left:5px;text-decoration:underline;font-weight:normal;font-size:10px;color:#800000;'), $lang['dataqueries_231'])
							) .
							RCView::div(array('style'=>'margin-left:45px;'), 
								...
								RCView::a(array('href'=>APP_PATH_WEBROOT."DataQuality/field_comment_log.php?pid=".PROJECT_ID,'style'=>'margin-left:15px;text-decoration:underline;font-weight:normal;font-size:10px;'), $lang['setup_53'])
							)
						) .
			...
			foreach ($rattr as $this_event_id=>$eattr) {
				foreach ($eattr as $this_field=>$fattr) {
					foreach ($fattr as $attr) {
						...
						if ($longitudinal) {
							$line = array($this_record, $Proj->eventInfo[$this_event_id]['name_ext'], $this_field, $attr['username'], $attr['ts'], $attr['comment']);
						} else {
							$line = array($this_record, $this_field, $attr['username'], $attr['ts'], $attr['comment']);
						}
	...
	public function deleteFieldComment($res_id)
	{
		...
		if (db_affected_rows() == 1) {			
			...
			// Log it
			log_event("","redcap_data_quality_resolutions","MANAGE",$row['record'],$logDataValues,"Delete field comment");
			return true;
		} else {
			return false;
		}
	...
	public function editFieldComment($res_id, $comment)
	{
		...
		if (db_query($sql)) {			
			...
			// Log it
			log_event("","redcap_data_quality_resolutions","MANAGE",$row['record'],$logDataValues,"Edit field comment");
			return true;
		} else {
			return false;
		}
