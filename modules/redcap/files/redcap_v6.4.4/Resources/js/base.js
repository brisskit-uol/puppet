...
function datediff(d1,d2,unit,dateformat,returnSigned) 
{
	...
	var d1isToday = (d1 == "today" || d1 == "TODAY" || d1 == today);
...
function displayFormSaveBtnTooltip() {
	// Do not do this for the mobile view
...
function filePopUp(field_name,label_page,signature) {
	...
	$.get(label_page, { s: getParameterByName('s'), field_name: field_name, event_id: event_id, record: getParameterByName('id') },
		function(data) {
			...
			$('#file_upload').dialog({ title: (signature == 1 ? lang_file_upload_title2 : lang_file_upload_title1), bgiframe: true, modal: true, width: (isMobileDevice ? $('#form_table').width() : 500) });
			// Signature?
			if (signature == 1) {
...
function stopUpload(success,this_field,doc_id,doc_name,study_id,doc_size,event_id,download_page,delete_page,doc_id_hash){
	var result = '';
	if (success == 1){
		...
		document.getElementById(this_field+"-link").href = download_page+"&doc_id_hash="+doc_id_hash+"&id="+doc_id+"&s="+getParameterByName('s')+"&record="+study_id+"&page="+getParameterByName('page')+"&event_id="+event_id+"&field_name="+this_field;
		$('#'+this_field+"-link").attr('onclick', "return appendRespHash('"+this_field+"');");
		var newlinktext = '<img src="'+app_path_images+'bullet_delete.png"> '
			+ '<a href="javascript:;" style="font-size:10px;color:red;" onclick=\'deleteDocumentConfirm('+doc_id+',"'+this_field+'","'+study_id+'",'+event_id+',"'+delete_page+'&__response_hash__="+$("#form input[name=__response_hash__]").val());return false;\'>Remove file</a>';
		if (sendit_enabled) {
		...
		if (sigimg.length) {
			sigimg.show().html('<img src="'+download_page.replace('file_download.php','image_view.php')+"&doc_id_hash="+doc_id_hash+"&id="+doc_id+"&s="+getParameterByName('s')+"&record="+study_id+"&page="+getParameterByName('page')+"&event_id="+event_id+"&field_name="+this_field+'&signature=1" alt="[SIGNATURE]">');
		}
	} else {
	...
	if (success == 1) {
		...
		if ($('#'+this_field+'-sigimg').length) {
			...
			setTimeout(function(){	
				$('#file_upload').dialog('option', 'hide', {effect:'fade', duration: 500}).dialog('close');
...
function deleteDocumentConfirm(doc_id,this_field,id,event_id,delete_page) {
	simpleDialog("Are you sure you want to permanently remove this document?","Delete document?",null,420,null,"Cancel",function(){
		deleteDocument(doc_id,this_field,id,event_id,delete_page);
	},"Yes, delete it");
}
function deleteDocument(doc_id,this_field,id,event_id,delete_page) {
	...
	$.get(delete_page, { s: getParameterByName('s'), id: doc_id, field_name: this_field, record: id, event_id: event_id },function(data) {
		...
		// Close dialog automatically with fade effect
		setTimeout(function(){	
			$('#'+file_delete_dialog_id).dialog('option', 'hide', {effect:'fade', duration: 500}).dialog('close');
			// Destroy the dialog so that fade effect doesn't persist if reopened
			setTimeout(function(){
				$('#'+file_delete_dialog_id).dialog('destroy');
			},500);
		},2200);
	});
	// Trigger branching logic in case a "file" field is involved in branching
...
function formSubmitDataEntry() {
	...
	// Re-enable any disabled fields (due to field note annotations and such)
	$('#form_table input:disabled, #form_table select:disabled, #form_table textarea:disabled').prop('disabled', false);
	// Submit form (finally!)
...
function initPage() {
	...
	$("#menuLnkEditInstr, #menuLnkEditReports").hover(function() {
...
function setInviteDeliveryMethod(ob) {
	...
	} else if (val == 'SMS_INVITE_MAKE_CALL' || val == 'SMS_INVITE_RECEIVE_CALL' || val == 'SMS_INITIATE') {
		...
		setTimeout(function(){
			...
			if (floatFirstRow) {
				...
				$('#'+table_id+' thead tr th').each(function(){
					...
					//alert($('#'+table_id).width()+', '+$(cloneClassRow+' table').width());
			}
			// If floating the first column, then perform extra adjustments
			if (floatFirstCol) {
...
function validateSurveyRemindersOptions() {
		...
		|| reminder_type == null) 
	{
		...
		// Display error msg
		simpleDialog("<div style='color:#C00000;font-size:13px;'><img src='"+app_path_images+"exclamation.png' class='imgfix'> ERROR: If you are enabling reminders, please make sure all reminder choices are selected. One or more options are not entered/selected.</div>", reminder_title, null, 400);
		return false;
	}
	return true;
}
...
function requestToken() {
	$.post(app_path_webroot +'API/project_api_ajax.php?pid='+pid,
		{ action: 'requestToken' },
		function (data) {
			simpleDialog(data);
		}
...
// Function that is run on forms and surveys to hide or disable fields based on annotation in the Field Note text
function triggerInstrumentAnnotations() {
	...
	// HIDES ANY INPUT THAT CONTAINS @HIDDEN IN THE NOTES FIELD
	// Useful for passing variables into a survey and keeping them hidden from end users
	var term = '@HIDDEN';
	var selector = "#form_table div.note:contains('"+term+"'), #form_table div.note:contains('"+term+"-" + (isSurvey ? "SURVEY" : "FORM") + "ONLY')";
	$(selector).each(function() {
		// Hide row
		$(this).parentsUntil($("tr")).parent().hide();
	});
	
	// DISABLES ANY INPUT WITH THE SEARCH TERM IN THE NOTE FIELD
	var term  = '@DISABLED';
	var terms = '@DISABLED-SURVEYONLY';
	var termf = '@DISABLED-FORMONLY';
	var selector = "#form_table div.note:contains('"+term+"'), #form_table div.note:contains('"+terms+"'), #form_table div.note:contains('"+termf+"')";
	$(selector).each(function() {
		var cthis = $(this);
		var cthis_html = cthis.html();
		// Which terms do we have in this field note?
		var terms_have = (cthis_html.indexOf(terms) > -1);
		if (terms_have) {
			cthis_html = cthis.html(cthis_html.replace(terms,'')).html();
		}
		var termf_have = (cthis_html.indexOf(termf) > -1);
		if (termf_have) {
			cthis_html = cthis.html(cthis_html.replace(termf,'')).html();
		}
		var term_have = (cthis_html.indexOf(term) > -1);
		if (term_have) {
			cthis_html = cthis.html(cthis_html.replace(term,'')).html();
		}
		// If field note no longer has anything in it, then hide it so it's not taking up space
		if (trim(cthis_html) == '') cthis.hide();
		// If this field note contains the term, then perform actions
		if (term_have || (!isSurvey && termf_have) || (isSurvey && terms_have)) {
			// Get row
			tr = cthis.parentsUntil($("tr")).parent();
			// Disable sliders
			if (tr.find('.slider').length) {
				var fld = tr.attr('sq_id');
				$("#sldrmsg-"+fld).css('visibility','hidden');
				$("#slider-"+fld).attr('onmousedown', '').slider("disable");
				// Set timeout for disabling the sliders since there might be later javascript for enabling them
				setTimeout(function(){
					$("#slider-"+fld).slider("disable");
				},100);
				setTimeout(function(){
			...
			tr.fadeTo(0, 0.5).find('input, select, textarea').prop("disabled", true);
			// Disable file upload links and "reset" links
			$('a', tr).each(function(){
				// Skip this link if it contains an image
				if (!$(this).find('img').length) {
					$(this).attr('href', 'javascript:;').attr('onclick', '').attr('onfocus', '');
				}
			});
		}
	});
...
// Display the Two Factor Auth login dialog (login form)
function display2FALoginDialog() {
	$('#two_factor_login_dialog').dialog({ bgiframe: true, modal: true, width: (isMobileDevice ? $('body').width() : 720), open:function(){fitDialog(this);}, 
...
function verify2FAcode(code) {
	...
	$.post(app_path_webroot+"Authentication/two_factor_verify_code.php",{ code: trim(code) },function(data) {
		if (data != '1') {
			simpleDialog("<div style='font-size:14px;'>"+lang2FA_01+"</div>",lang2FA_03,null,400);
		} else {
