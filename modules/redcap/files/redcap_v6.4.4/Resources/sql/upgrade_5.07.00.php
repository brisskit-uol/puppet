...
if (getDecVersion($current_version) != 50020 && getDecVersion($current_version) != 50021 && getDecVersion($current_version) != 50022) {
	...
	while ($row = db_fetch_assoc($q))
	{
		...
		$sql2 = "select timestamp(ts) from redcap_log_event where user = '".prep($row['username'])."' order by log_event_id limit 1";
		$q2 = db_query($sql2);
		if (db_num_rows($q2) > 0) {
			$ts = db_result($q2, 0);
...
{
	// Get first activity timestamp
	$sql2 = "select timestamp(ts) from redcap_log_event where user = '".prep($row['username'])."' order by log_event_id desc limit 1";
	$q2 = db_query($sql2);
	if (db_num_rows($q2) > 0) {
		$ts = db_result($q2, 0);
...
while ($row = db_fetch_assoc($q))
{
	...
	$sql2 = "select timestamp(ts) from redcap_log_view where user = '".prep($row['username'])."' 
			and session_id = (select session_id from redcap_log_view where user = '".prep($row['username'])."' order by log_view_id desc limit 1) 
