...
<h2>Introduction to Hooks & Plugins</h2>

<p>This is the official documentation for developers of REDCap hooks and REDCap plugins. 
It provides an introduction to the basics
of what hooks and plugins are and how they might be used. It includes FAQ pages for frequently asked questions regarding
hooks and plugins, and it also serves as documentation for all the official REDCap developer methods that may be used in a REDCap hook and plugin.</p>

<p>
REDCap hooks and plugins serve the purpose of allowing one to extend and add to REDCap's functionality without
...
viewed as a very prudent thing to do. But hooks and plugins allow one to add new features and functionality to
REDCap without having to modify the REDCap code, and another benefit of utilizing hooks and plugins is that their code
does not have to be modified as you upgrade to new versions of REDCap.
</p>


<h2>What are hooks and plugins? How are they different?</h2>
<p>
REDCap hooks and plugins are very different animals, so to speak, and serve very different purposes. 
Both are PHP scripts utilized by REDCap, in which a programmer will write the hooks and plugins,
and then those scripts will be placed on the REDCap web server so that they can be accessed by REDCap. 
Hooks and plugins are meant to be
written by a programmer who is part of (or associated with) the REDCap team at the local institution.
Hooks and plugins have the ability to access both your web server and database server directly, so 
they will have access to your web server's file system and all REDCap data stored in MySQL.
Thus only trusted, high-level individuals should be given the ability to author hooks and plugins.
While the REDCap API is a user-level feature, hooks and plugins are not. Although they may indeed get used
by end-users, hooks and plugins will not be created by end-users. (If hooks or plugins are ever created by 
...
<a target="_blank" style="text-decoration:underline;" href="https://iwg.devguard.com/trac/redcap/wiki/PluginLibrary">Plugin Library</a> 
...
by filling out the <a target="_blank" style="text-decoration:underline;" href="http://tinyurl.com/redcapsupport2">Request for Access to REDCap Consortium Support Tools survey</a>
for you.
</p>
...
<h2>What are REDCap developer methods?</h2>

<p>The static PHP methods listed on the left-hand menu of this page are the official REDCap developer methods
that can be utilized by both plugins and hooks to allow you to perform common 
